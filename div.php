...
if (!defined('DIV_REPO')) define('DIV_REPO', './');
...
define('DIV_TEMPLATE_FOR_DOCS', '@_DIALECT = ' . uniqid() . base64_decode('PGh0bWw+DQoJPGhlYWQ+DQoJCTx0aXRsZT57JHRpdGxlfTwvdGl0bGU+DQoJCTxzdHlsZSB0eXBl
...
class div
{
    ...
    public function __construct($src = null, $items = null, $ignore = [])
    {
        ...
        if (self::$__log_mode) $this->logger('Building instance #' . $this->__id . ' of ' . $class_name . '...');
        ...
        if (is_null($src)) {
            if ($class_name != self::$__super_class && is_null($this->__src)) $src = $class_name;
        ...
        if (is_object($items)) {
            if (method_exists($items, '__toString')) {
                $item_str = "$items";
                if (!isset ($items->value)) $items->value = $item_str;
                $items->_to_string = $item_str;
            }
            $items = get_object_vars($items);
        }

        if (!$discardFileSystem) $src = $this->loadTemplate($src);
        ...
        if (isset ($ignore [0])) foreach ($ignore as $key => $val) {
        ...
        if (!isset($this->__items['div'])) $this->__items['div'] = [];
     ...
     * @return array
     */
    final static function getIncludePaths($packages = PACKAGES, $repo = DIV_REPO)
    {
    ...
    {
        if (is_null($class_name)) $class_name = get_class($this);
        $parent = get_parent_class($class_name);

        if ($parent === false) return $class_name;
     ...
     * @param object $from
     */
    final static function createAuxiliaryEngine(&$from = null)
    {
        if (is_null($from)) $class_name = self::$__super_class; // auxiliary engine is brother of $__super_class
        else $class_name = get_class($from); // auxiliary engine is brother of $from

        if (!is_null(self::$__engine)) if (get_class(self::$__engine) != $class_name) self::$__engine = null;

        if (is_null(self::$__engine)) {
            if (self::$__log_mode) self::log("createAuxiliaryEngine: A new $class_name instance will be created ...");
            ...
            if (!is_null($from)) // auxiliary engine have optional items
            {
     ...
     * @return div
     */
    final static function getAuxiliaryEngineClone(&$items = null, &$items_orig = null, $from = null, $src = null, $src_orig = null)
    {
        if (!is_null($from)) {
            if (!is_null(self::$__engine) && get_class($from) == get_class(self::$__engine)) $obj = clone self::$__engine;
            else {
                $temp_engine = null;
            ...
            if (is_null(self::$__engine)) self::createAuxiliaryEngine($from);
            $obj = clone self::$__engine;
        }
        ...
        if (!is_null($src)) $obj->__src = $items;
        if (!is_null($src_orig)) $obj->__src_orig = $items_orig;
    ...
    final public function saveOperation($params = [])
    {
        ...
        if (!isset(self::$__remember[$this->__crc])) self::$__remember[$this->__crc] = [];
    ...
    {
        foreach ($memories as $k => $v) self::$__remember[$k] = $v;
    }
    ...
    final static function getSystemData()
    {
        ...
        if (self::$__system_data == null) {
            ...
            if (isset (self::$__system_data_allowed ["div{$d}now"])) self::$__system_data ["div{$d}now"] = time();
            if (isset (self::$__system_data_allowed ["div{$d}post"])) self::$__system_data ["div{$d}post"] = $_POST;
            if (isset (self::$__system_data_allowed ["div{$d}get"])) self::$__system_data ["div{$d}get"] = $_GET;
            if (isset (self::$__system_data_allowed ["div{$d}server"])) self::$__system_data ["div{$d}server"] = $_SERVER;
            if (isset (self::$__system_data_allowed ["div{$d}session"])) self::$__system_data ["div{$d}session"] = isset ($_SESSION) ? $_SESSION : [];
            if (isset (self::$__system_data_allowed ["div{$d}version"])) self::$__system_data ["div{$d}version"] = self::$__version;
     ...
     */
    final static function setAllowedFunction($function_name)
    {
     ...
     * @param string $function_name
     */
    final static function unsetAllowedFunction($function_name)
    {
    ...
    {
        $id = serialize($search);
        if (!isset (self::$__defaults_by_var [$var])) self::$__defaults_by_var [$var] = [];
    ...
    {
        if (self::$__log_mode === true) $this->logger("Loading the template: $template_path");
        ...
        if (strlen($template_path) < 255) {
            ...
            foreach ($paths as $path) {
                if (strlen($path) < 255) if (self::fileExists($path)) {
    ...
    final public function changeTemplate($src = null)
    {
        ...
        if (!$discard_fs) $src = $this->loadTemplate($src);
    ...
    final public function setItem($var, $value = null)
    {
        if (is_array($var)) {
            ...
            foreach ($var as $idx => $val) {
                if (self::issetVar($idx, $this->__items)) $r [$idx] = self::getVarValue($idx, $this->__items);
                else
                    $r [$idx] = null;
        ...
        if (self::issetVar($var, $this->__items)) $item = self::getVarValue($var, $this->__items);
        else
            $item = null;
    ...
    final public function getRanges($tag_ini, $tag_end, $src = null, $only_first = false, $pos = 0)
    {
        ...
        if (!is_null($src)) if (isset ($src [0]) && !empty ($src)) {
            ...
            do {
                ...
                if (isset ($src [$pos])) $ini = strpos($src, $tag_ini, $pos);

                if ($ini !== false) {
                    if (isset ($src [$ini + $tag_ini_len])) {
                        ...
                        if ($fin !== false) {
                            ...
                            while (true) {
                                ...
                                while (true) {
                                    ...
                                    if ($open === false && $close !== false && $tag_pos_right === $last_tag_pos_right) break; // close and not open
                                    if ($open !== false && $close === false && $tag_pos_right === $last_tag_pos_right) break; // open and not close
            ...
            if ($prefix_pos !== false) {
                if (isset ($src [$prefix_pos + 1])) {
                    ...
                    if ($begin_suffix != '' && !is_null($begin_suffix)) {
                        ...
                        $suffix_pos = false;
                        if (count($stop_pos) > 0) $suffix_pos = min($stop_pos);
                    }

                    $key = '';
                    if ($suffix_pos < $l && $suffix_pos !== false) $key = substr($src, $prefix_pos + $l1, $suffix_pos - $prefix_pos - $l1);
                    ...
                    if ($key !== '' && !isset ($tags_done [$key])) {
                        ...
                        // Only the first...
                        if ($only_first) if (isset ($ranges [0])) break;
    ...
    final public function searchInListRanges($pos = 0)
    {
        ...
        foreach ($ranges as $rang) if ($pos > $rang [0] && $pos < $rang [1]) return true;
    ...
    final public function searchPosAfterRange($tag_begin, $tag_end, $pos)
    {
        ...
        if (isset ($ranges [0])) if ($ranges [0] [0] < $pos) return true;
    ...
    final public function getElseTag($sub_src)
    {
        ...
        do {
            $continue = false;
            if ($else_pos < $ls) $else = strpos($sub_src, DIV_TAG_ELSE, $else_pos);
            else
                $else = false;
    ...
    final public function getEmptyTag($sub_src)
    {
        ...
        do {
            $continue = false;
            if ($empty_pos < $ls) $empty = strpos($sub_src, DIV_TAG_EMPTY, $empty_pos);
            else
                $empty = false;
    ...
    final public function parseSubMatch(&$items, $key, $value, $modifiers = [])
    {
        ...
        if (strpos($this->__src, $key . DIV_TAG_SUBMATCH_SEPARATOR) !== false) foreach ($modifiers as $modifier) {
            while (true) {
                ...
                if (self::$__log_mode) $this->logger("Parsing sub-match  $tag_begin");
                ...
                if (!is_numeric($arr [0]) || !is_numeric($arr [1])) {
                    if (substr("{$arr[1]}", 0, 1) == DIV_TAG_MODIFIER_TRUNCATE) {
                        $items [$new_key] = $vpx . self::teaser("{$value}", intval(substr($arr [1], 1))) . $vsx;
                        $this->saveOperation([
                        ...
                        $items [$new_key] = $vpx . wordwrap("{$value}", intval(substr($arr [1], 1)), "\n", 1) . $vsx;
                        $this->saveOperation([
                    ...
                    $items [$new_key] = $vpx . substr("$value", $arr [0], $arr [1]) . $vsx;
                    $this->saveOperation([
        ...
        if (strpos($this->__src, "$key" . DIV_TAG_VAR_MEMBER_DELIMITER) !== false) {
            ...
            if (is_array($value)) foreach ($value as $kk => $v) {
                $this->parseSubMatch($items, $key . DIV_TAG_VAR_MEMBER_DELIMITER . $kk, $v, $modifiers);
            }
        }
    ...
    {
        if (self::$__log_mode) $this->logger("Parsing sub-matches...");
        if (is_null($items)) $items = &$this->__items;
    ...
    {
        if (is_string($var)) $var = explode(" ", str_replace(",", " ", $var));

        $literals = self::getVarValue('div' . DIV_TAG_VAR_MEMBER_DELIMITER . 'literals', self::$__globals_design);

        if (is_null($literals) || $literals === false) $literals = [];

        if (is_string($literals)) $literals = explode(" ", str_replace(",", " ", $literals));
    ...
    final public function getLiterals()
    {
        ...
        if (is_null($val) || $val === false) return [];

        if (is_string($val)) $val = explode(" ", str_replace(",", " ", $val));
    ...
    final public function parseMatch($key, $value, &$engine, $ignore_logical_order = false)
    {
        ...
        if ($is_string || is_numeric($value)) {
            ...
            if ($p1 === false && $p2 === false) $sub_str = $this->__src;
            elseif ($p1 !== false && $p2 !== false) {
                $min = min($p1, $p2);
                $sub_str = substr($this->__src, 0, $min);
                $p1 = $min;
            } elseif ($p1 !== false && $p2 === false) $sub_str = substr($this->__src, 0, $p1);
            else {
            ...
            if ($px !== false) {
                ...
                $replace_count = 0;
                if (self::$__log_mode) $this->logger("Parsing match: $key in '" . substr($sub_str, 0, 50) . "'");
                ...
                if (trim($value) != "" && $literal === false) {
                    ...
                    if (self::issetVar($key, $this->__items) && crc32($value) != $crc) {
                        if (gettype(self::getVarValue($key, $this->__items)) == "string") self::setVarValue($key, $value, $this->__items);
                    }
                }
                ...
                if ($replace_count > 0 && !isset (self::$__dont_remember_it [$key])) $this->saveOperation([
                ...
                foreach (self::$__custom_modifiers as $modifier) {
                    if (strpos($sub_str, $modifier[0]) === false) continue;
                    ...
                    if ($proceed) $sub_str = str_replace($prefix . $modifier [0] . $key . $suffix, $vpx . $callable_result . $vsx, $sub_str);
                }

                if ($p1 !== false) $this->__src = $sub_str . substr($this->__src, $p1);
                else
                    $this->__src = $sub_str;
            }
        }
    ...
    final public function parseIterations(&$items, &$src = null)
    {
        ...
        while (true) {
            ...
            if (is_numeric($range [0]) && is_numeric($range [1])) {
                ...
                if (strpos($sub_src, DIV_TAG_LOOP_VAR_SEPARATOR)) $iterator_var_code = '';
                ...
                if ($range[1] >= $range [0]) for ($i = $range[0]; $i >= $range[0] && $i <= $range[1]; $i = $i + $step) $items[$key][] = $i;
                    ...
                    for ($i = $range[0]; $i >= $range[1] && $i <= $range[0]; $i = $i - $step) $items[$key][] = $i;
            }
        }
    }
    ...
    {
        if (is_null($ranges)) $ranges = $this->getRanges($tag_ini, $tag_end);

        foreach ($ranges as $rang) if ($rang [0] < $pos1 && $rang [1] > $pos2) return true;
            ...
            if ($p2 > $p1) {
                ...
                if (empty ($value)) $h = $body_parts [1];
                    ...
                    foreach ($value as $kk => $item) {
                        ...
                        if ($go_is_last) $anothers ['_is_last'] = ($ii == $keys_count);
                        if ($go_list) $anothers ['_list'] = $key;

                        if ($go_item) {
                            if (is_object($item)) $anothers ['_item'] = clone $item;
                            else
                                $anothers ['_item'] = $item;
                        }
                    ...
                    // Parsing ...
                    $h = "";
                    $engine = self::getAuxiliaryEngineClone($xitems, $xitems, $this);
                    ...
                    foreach ($xitems as $xkey => $item) {
                        ...
                        foreach ($item as $kkk => $vvv) if (isset (self::$__globals_design [$kkk])) unset (self::$__globals_design [$kkk]);
                        ...
                        foreach ($item as $kkk => $vvv) if (isset ($engine->__memory [$kkk])) unset ($engine->__memory [$kkk]);
                        ...
                        foreach ($item as $kkk => $vvv) if (isset ($this->__memory [$kkk])) unset ($this->__memory [$kkk]);
    ...
    final public function parseList($items = null, $superkey = "")
    {
        ...
        if ($superkey != "") $superkey .= DIV_TAG_VAR_MEMBER_DELIMITER;
    ...
    final public function parseIgnore()
    {
        ...
        // Generate internal and random ignore tag (security reasons)
        if (is_null($this->__ignore_secret_tag)) $this->__ignore_secret_tag = uniqid();
    ...
    final public function getTplPath($path)
    {
        ...
        if (is_null($return)) {
            ...
            foreach (self::$__includes_history as $ih) {
                ...
                if (self::fileExists($full_path . "." . DIV_DEFAULT_TPL_FILE_EXT)) $full_path .= "." . DIV_DEFAULT_TPL_FILE_EXT;
                elseif (self::fileExists($full_path . "." . DIV_DEFAULT_TPL_FILE_EXT)) $full_path .= "." . DIV_DEFAULT_TPL_FILE_EXT;

                $similar = similar_text($ih, $full_path);
                if ((self::fileExists($full_path) || self::fileExists($full_path)) && $similar >= $max) $return = $full_path;
            }
        }

        if (!self::fileExists($return)) return null;
    ...
    final public function parseInclude(&$items)
    {
        ...
        while (true) {
            ...
            if (!self::fileExists($path)) {
                ...
                if ($sep !== false) {
                    ...
                    if (is_object($data_path)) $data_path = get_object_vars($data_path);

                    if (is_array($data_path)) $items = array_merge($items, $data_path);
                }
            }

            $path = $this->getTplPath($path);

            if (self::$__log_mode) $this->logger("Trying to include $path");
            ...
            if ($path != $this->__path && !self::detectRecursiveInclusion($exclusion, $path, $ini)) {
                if (self::fileExists($path)) {
                    ...
                    if (!is_dir($path)) {
                        ...
                        if (!is_null($data_path)) {
                            ...
                            if (isset ($data_path ['from'])) {
                                if (isset ($data_path ['to'])) {
                                    ...
                                    if (!is_numeric($from) && !is_numeric($to)) if (!empty ($from) && !empty ($to)) {
                                        ...
                                        foreach ($extracts as $extract) {
                                            ...
                                            if (isset ($data_path ['offset'])) if ($i < $data_path ['offset'] * 1) continue;

                                            $new_c .= substr($c, $extract [0] + $from_len, $extract [1] - $extract [0] - $from_len);

                                            if (isset ($data_path ['limit'])) if ($i == $data_path ['limit'] * 1) break;
                                        }

                                        $c = $new_c;
                                    } else {
                                        // numeric/string from/to
                                        if (!is_numeric($from)) if (!empty ($from)) $from = strpos($c, $from);
                                        else
                                            $from = false;
                                        if (!is_numeric($to)) if (!empty ($to)) $to = strpos($c, $to);
                                        else
                                            $to = false;
                                    ...
                                    if (!is_numeric($from)) $from = strpos($c, $from);
                                ...
                                if (isset ($data_path ['to'])) {
                                    $to = $data_path ['to'];
                                    if (!is_numeric($to)) $to = strpos($c, $to);
                        ...
                        if (self::$__docs_on) {
                            if (self::fileExists($this->__path) || self::fileExists(PACKAGES . $this->__path)) {
                                ...
                                if (!isset (self::$__docs [$section])) self::$__docs [$section] = [];
                                if (!isset (self::$__docs [$section] ['include'])) self::$__docs [$section] ['include'] = [];
                    ...
                    foreach ($exclusion as $idx => $exc) {
                        if ($exc ['ini'] > $ini) $exclusion [$idx] ['ini'] += $c_len;
                        if ($exc ['end'] > $ini) $exclusion [$idx] ['end'] += $c_len;
                    }
        ...
        foreach ($restores as $id => $restore) $this->__src = str_replace($id, $restore, $this->__src);
    }
    ...
    final public function parsePreprocessed($items)
    {
        ...
        while (true) {
            ...
            // Div 5.0: Do not include anything within the conditional
            ...
            if (!self::fileExists($path)) {
                ...
                if ($sep !== false) {
                    ...
                    if (is_null($standalone)) $standalone = false;

                    if ($standalone) $items = $data_path;
                    elseif (is_array($data_path)) $items = array_merge($items, $data_path);
                }
            }
            ...
            if ($path == $this->__path /*|| self::detectRecursiveInclusion($exclusion, $path, $ini)*/) if (self::$__log_mode) self::log("Recursive inclusion of template '$path' in '" . substr($this->__src, $ini - 20, 20) . "' during pre-process.", DIV_ERROR_WARNING);
            ...
            if (self::fileExists($path)) {
                ...
                if (self::$__docs_on) {
                    if (self::fileExists($this->__path)) {
                        ...
                        self::$__docs [$contained] = [];
                        if (!isset (self::$__docs [$section])) self::$__docs [$section] = [];
                        if (!isset (self::$__docs [$section] ['preprocess'])) self::$__docs [$section] ['include'] = [];
                        ...
                        if ($line [0] == '@') {
                            ...
                            if (isset (self::$__docs [$section] [$prop])) {
                                ...
                                if (!is_array(self::$__docs [$section] [$prop])) {
                                    if (trim(self::$__docs [$section] [$prop]) !== '') self::$__docs [$section] [$prop] = [
                                        self::$__docs [$section] [$prop]
                                    ]; else
                                        self::$__docs [$section] [$prop] = [];
                                }

                                if (isset (self::$__docs [$section] [$prop] [0])
                                    || (!isset (self::$__docs [$section] [$prop] [0])
                                        && trim($value) !== ''))
    ...
    {
        if (is_null($items)) $items = $this->__memory;

        $vars = [];
        $items_x = [];

        if (is_object($items)) $items_x = get_object_vars($items);
        elseif (is_array($items)) $items_x = $items;
        else
            return [];
        ...
        foreach ($items_x as $key => $value) {
            ...
            if ($x_key !== '') {
                ...
                if (!is_scalar($value) && !is_null($value)) $vars = array_merge($vars, $this->getVars($items_x [$key], $x_key . DIV_TAG_VAR_MEMBER_DELIMITER));
            }
        }
    ...
    final static function getVarValue($var, $items)
    {
        ...
        if (isset ($items_x [$var])) return $items_x [$var];
        ...
        while (true) {
            ...
            if ($ni !== false) {
                ...
                if (is_array($temp_value)) {
                    if (!isset (self::$__globals_design [$var])) $setup_design = false;
                            ...
                            foreach ($vars as $kkk => $vvv) {
                                if (self::isString($vvv)) {
                                    ...
                                    if (isset ($vvv [0])) if ($vvv [0] == "$") {
                                        $var_x = substr($vvv, 1);
                                        if (self::issetVar($var_x, $items)) {
                                            if (is_array($value)) $value [$kkk] = self::getVarValue($var_x, $items);
                                            if (is_object($value)) $value->$kkk = self::getVarValue($var_x, $items);
                                        }
                                    }
                                }
                            }
            ...
            if ($setup == true) {
                ...
                if ($setup_design) self::setVarValue($var, $value, self::$__globals_design);
            }
    ...
    final public function parseDefaults(&$items)
    {
        ...
        while (true) {
            ...
            if (self::fileExists($replace) && !self::isDir($search)) $replace = self::jsonDecode(self::getFileContents($replace), $this->getAllItems($items));
            ...
            if (self::fileExists($search) && !self::isDir($search)) $search = self::jsonDecode(self::getFileContents($search), $this->getAllItems($items));
            if (self::fileExists($search . "." . DIV_DEFAULT_DATA_FILE_EXT) && !self::isDir($search . "." . DIV_DEFAULT_DATA_FILE_EXT)) $search = self::jsonDecode(self::getFileContents($search . "." . DIV_DEFAULT_DATA_FILE_EXT), $this->getAllItems($items));
    ...
    {
        if (is_null($items)) $items = &$this->__items;
        if (is_null($engine)) $engine = self::getAuxiliaryEngineClone($items, $items, $this);
        ...
        if (strpos($this->__src, $key . DIV_TAG_VAR_MEMBER_DELIMITER) !== false) {
            ...
            if (is_array($vars)) foreach ($vars as $kk => $v) {
                    ...
                    foreach ($parts as $part) {
                        if (trim($part) !== "") {
                            ...
                            elseif (strpos($part, DIV_TAG_MODIFIER_TRUNCATE) === 0 || strpos($part, DIV_TAG_MODIFIER_WORDWRAP) === 0 || strpos($part, ",") !== false) {
                                $new_code .= DIV_TAG_TPLVAR_BEGIN . ' ' . $new_var . DIV_TAG_TPLVAR_ASSIGN_OPERATOR . ' ' . DIV_TAG_REPLACEMENT_PREFIX . DIV_TAG_MODIFIER_SIMPLE . $new_var . DIV_TAG_SUBMATCH_SEPARATOR . $part . DIV_TAG_REPLACEMENT_SUFFIX . ' ' . DIV_TAG_TPLVAR_END . "\n";
                            } else {
                                $break = true;
                            }
    ...
    final public function parseMatches(&$items = null, $ignore_logical_order = false)
    {
        ...
        if (strpos($this->__src, DIV_TAG_LOOP_BEGIN_PREFIX) !== false) {
            ...
            while (true) {
                if ($last_pos > strlen($this->__src) - 1) break;
        ...
        while (true) {
            ...
            if (count($ranges) > 0) {
                ...
                if (is_null($r)) {
                    ...
                    $this->__src = substr($this->__src, 0, $ini) . '{' . "$restore_id" . '}' . substr($this->__src, $fin + $suffix_len);
                    continue;
                }
                    ...
                    if (self::issetVar($arr [0], $items)) {
                        ...
                        if (is_array($v)) {
                            foreach ($v as $kk => $iv) {
                                ...
                                if (isset ($vars [$vv])) $vars [$otag [2]] = self::mixedBool($vars [$otag [2]]) || self::mixedBool($vv);
                                else
    ...
    final static function haveDivCode($src)
    {
        ...
        foreach ($all_tags as $tag) {
            if ($tag !== '') if (strpos($src, $tag) !== false) return true;
        }
    ...
    final public function getConditionalRanges($orphans = true, $src = null, $strict = false)
    {
        ...
        if (!$orphans) {
            $nranges = [];
            foreach ($ranges as $rang) {
                if (self::varExists($rang [2], $this->__items)) $nranges [] = $rang;
            }
            $ranges = $nranges;
        }
        ...
        if ($strict !== false) {
            $nranges = [];
            foreach ($ranges as $rang) {
                if (!self::haveDivCode($rang [2])) $nranges [] = $rang;
            }
            $ranges = $nranges;
        }
        ...
        while (true) {
            ...
            if (is_object($value)) {
                if (method_exists($value, '__toString')) {
                    $item_str = "$value";
                    if (!isset ($value->value)) $value->value = $item_str;
            ...
            if (is_array($value)) foreach ($value as $kkk => $vvv) {
            ...
            if (is_array($this->__items_orig)) {
                if (isset ($this->__items_orig [$key])) $engine->__items_orig = $this->__items_orig [$key];
    ...
    final public function getMethodResult($code, &$items = null)
    {
        ...
        foreach ($objects as $obj) {
            ...
            foreach ($methods as $m) {
                if (array_search($m, self::$__parent_method_names) === false) $ms [] = $m;
            }
    ...
    final public function getActiveVars($items, $super_key = '', $src = null)
    {
        ...
        if ($super_key != '' && strpos($src, $super_key) === false) return [];
        ...
        foreach ($items_x as $key => $value) {
            if ($super_key . $key !== '') if (strpos($src, $super_key . $key) !== false) {
                $vars [] = $super_key . $key;
                if (!is_scalar($items_x [$key])) $vars = array_merge($vars, $this->getActiveVars($items_x [$key], $super_key . $key . DIV_TAG_VAR_MEMBER_DELIMITER, $src));
            }
        }
    ...
    final private function memory(&$items)
    {
        ...
        foreach ($vars as $var) {
            if (!isset ($items [$var]) || strpos($var, DIV_TAG_VAR_MEMBER_DELIMITER) !== false) $items [$var] = self::getVarValue($var, $items);
        }
    ...
    final public function getLocations()
    {
        ...
        foreach ($r as $item) {
            ...
            if (!isset ($locations [$tag_name])) $locations [$tag_name] = [];
    ...
    final public function parseLocations()
    {
        ...
        foreach ($locations as $location => $positions) {
            ...
            while (true) {
                ...
                if ($new_content != "") {
                    if ($new_content [0] == ' ') $new_content = substr($new_content, 1);
                    if (substr($new_content, -1) == ' ') $new_content = substr_replace($new_content, "", -1);
                }
                ...
                foreach ($locations as $k => $v) foreach ($v as $kk => $p) if ($p > $end) $locations [$k] [$kk] -= $l1 + $l2 + ($end - $ini - $l1);
            }
    ...
    {
        $locations = $this->getLocations();
        foreach ($locations as $location => $positions) $this->__src = str_replace(DIV_TAG_LOCATION_BEGIN . $location . DIV_TAG_LOCATION_END, '', $this->__src);
    }
    ...
    final public function parseSubParsers(&$items = null, $flags = [])
    {
        ...
        foreach (self::$__sub_parsers as $parser => $function) {
            ...
            while (true) {
                ...
                if (self::$__log_mode && $p == 0) $this->logger("Parsing the sub-parser $parser ...");
            ...
            if (strpos($this->__src, $parser_tag_ini) !== false && !$ignore) {
                ...
                if ($proceed) $this->__src = str_replace($parser_tag_ini, $r, $this->__src, $replace_count);
            }
        }
    ...
    final public function checkLogicalOrder($ini = 0, $var = "", $check_loops = false, $check_matches = false, $check_formats = false, $check_data = false)
    {
        ...
        if ($check_matches) foreach (self::$__modifiers as $m) {
    ...
    final public function parseMacros(&$items = null, $ignore_previous_match = false)
    {
        ...
        while (true) {
            ...
            if (self::isValidMacro($this->__temp ['code'])) {
                ...
                foreach ($items as $key => $value) {
                    if (self::isValidVarName($key)) $this->__temp ['codevars'] .= '$' . $key . ' = $items["' . $key . '"];';
                }
                ...
                foreach ($this->__temp ['vars'] as $var => $value) {
                    ...
                    if (!isset ($items [$var]) || isset (self::$__globals_design [$var])) self::$__globals_design [$var] = $value;
                    ...
                    if ($s !== "") {
                        ...
                        if (count($arr) > 1) {
                            ...
                            if (!isset ($properties [$var])) {
                                ...
                                if (!is_null($value_json)) $value = $value_json;
                                else
                                    $value = trim($value);
    ...
    final public function parse($from_original = true, $item_index = null, $min_level = 1)
    {
        ...
        if (trim($this->__src) != "") {
            if (!is_null($item_index)) {
                if (self::$__log_mode) $this->logger("Parsing with '$item_index' index of __items");
            ...
            foreach ($props as $prop => $value) {
                if (substr($prop, 0, 2) != "__") $items [$prop] = $value;
            }

            // Reserved vars
            if (!isset ($items ['_empty'])) $items ['_empty'] = [];
            if (!isset ($items ['_'])) $items ['_'] = [];

            if (strpos($this->__src, DIV_TAG_SUBPARSER_BEGIN_PREFIX) !== false) $this->parseSubParsers($items, [
            ...
            if (strpos($this->__src, DIV_TAG_IGNORE_BEGIN) !== false || strpos($this->__src, '{' . $this->__ignore_secret_tag . '}') !== false) $this->parseIgnore();
            ...
            do {
                ...
                do {
                    ...
                    // Conditional
                    if (strpos($this->__src, DIV_TAG_CONDITIONAL_TRUE_BEGIN_PREFIX) !== false || strpos($this->__src, DIV_TAG_CONDITIONAL_FALSE_BEGIN_PREFIX) !== false) $this->parseConditional($items);
                    ...
                    if (strpos($this->__src, DIV_TAG_MULTI_MODIFIERS_PREFIX) !== false && strpos($this->__src, DIV_TAG_MULTI_MODIFIERS_SUFFIX) !== false) $this->parseMultipleModifiers();

                    // Data in templates
                    if (strpos($this->__src, DIV_TAG_TPLVAR_BEGIN) !== false) if (strpos($this->__src, DIV_TAG_TPLVAR_END) !== false) {
                        $items = array_merge($this->__memory, $items);
                    ...
                    // Lists
                    if (strpos($this->__src, DIV_TAG_LOOP_BEGIN_PREFIX) !== false) if (strpos($this->__src, DIV_TAG_LOOP_END_PREFIX) !== false) if (strpos($this->__src, DIV_TAG_LOOP_END_SUFFIX) !== false) {
                    ...
                    // Capsules
                    if (strpos($this->__src, DIV_TAG_CAPSULE_BEGIN_PREFIX) !== false) if (strpos($this->__src, DIV_TAG_CAPSULE_END_SUFFIX) !== false) $this->parseCapsules($items);
                    ...
                    // Matches
                    if (self::atLeastOneString($this->__src, self::$__modifiers) || (strpos($this->__src, DIV_TAG_NUMBER_FORMAT_PREFIX) !== false && strpos($this->__src, DIV_TAG_NUMBER_FORMAT_SUFFIX) !== false)) $this->parseMatches($items, $last_action);
                    ...
                    // Iterations
                    if (strpos($this->__src, DIV_TAG_ITERATION_BEGIN_PREFIX) !== false) if (strpos($this->__src, DIV_TAG_ITERATION_END) !== false) $this->parseIterations($items);

                    $crc_now = crc32($this->__src);

                    if ($checksum != $crc_now) $last_action = false;
                } while ($checksum != $crc_now);

                // Computing
                if (strpos($this->__src, DIV_TAG_FORMULA_BEGIN) !== false) if (strpos($this->__src, DIV_TAG_FORMULA_END) !== false) $this->parseFormulas($items);

                // Date format
                if (strpos($this->__src, DIV_TAG_DATE_FORMAT_PREFIX) !== false) if (strpos($this->__src, DIV_TAG_DATE_FORMAT_SUFFIX) !== false) $this->parseDateFormat($items);

                // Multiple replacements
                if (strpos($this->__src, DIV_TAG_MULTI_REPLACEMENT_BEGIN_PREFIX) !== false) if (strpos($this->__src, DIV_TAG_MULTI_REPLACEMENT_END_PREFIX) !== false) if (strpos($this->__src, DIV_TAG_MULTI_REPLACEMENT_END_SUFFIX) !== false) $this->parseMultiReplace($items);
            ...
            if (strpos($this->__src, DIV_TAG_CONDITIONAL_TRUE_BEGIN_PREFIX) !== false || strpos($this->__src, DIV_TAG_CONDITIONAL_FALSE_BEGIN_PREFIX) !== false) $this->parseOrphanParts();
            ...
            // Locations
            if (strpos($this->__src, DIV_TAG_LOCATION_CONTENT_BEGIN_PREFIX) !== false) if (strpos($this->__src, DIV_TAG_LOCATION_BEGIN) !== false) $this->parseLocations();
            ...
            if (strpos($this->__src, DIV_TAG_LOCATION_BEGIN) !== false) {
                ...
                if (is_null($clear)) $clear = true;

                if ($clear) $this->clearLocations();
            }
            ...
            if (self::$__parse_level <= 1) {
                ...
                foreach (self::$__ignored_parts as $id => $ignore) {
                    ...
                    foreach (self::$__sub_parsers as $sub_parser => $function) {
                        ...
                        if ($replaces_count > 0) $sub_parsers_restore [$tag_search] = $tag_replace;
                    }
            ...
            foreach ($sub_parsers_restore as $tag_search => $tag_replace) $this->__src = str_replace($tag_replace, $tag_search, $this->__src);
        }
    ...
    final public function txt()
    {
        ...
        while (true) {
            ...
            if ($width_separator_pos !== false) {
                $width = intval(trim(substr($sub_src, 0, $width_separator_pos)));
     ...
     * @return string
     */
    final private function translateSimpleBlocks($src, $begin_tag, $end_tag, $translated_begin_tag, $translated_end_tag, $separator_tag = "", $translated_separator_tag = "", $first = true)
    {
        $begin_tag_len = strlen($begin_tag);
        $end_tag_len = strlen($end_tag);
        $separator_tag_len = strlen(trim("$separator_tag"));
        $p = 0;
        while (true) {
            ...
            if ($separator_tag_len > 0) {
                if ($first) $po = strpos($sub_src, $separator_tag);
                else
                    $po = strrpos($sub_src, $separator_tag);

                if ($po !== false) $sub_src = substr($sub_src, 0, $po) . $translated_separator_tag . substr($sub_src, $po + $separator_tag_len);
            }
     ...
     * @return string
     */
    final private function translateKeyBlocks($src, $begin_prefix, $begin_suffix, $end_prefix, $end_suffix, $translated_begin_prefix, $translated_begin_suffix, $translated_end_prefix, $translated_end_suffix, $var_member_delimiter, $translated_var_member_delimiter)
    {
        $p = 0;
        while (true) {
    ...
    final public function translate($dialectFrom = [], $dialectTo = [], $src = null, $items = null)
    {
        ...
        if (self::$__log_mode === true) $this->logger("Translating template...");
        ...
        foreach ($constants as $c => $v) if (substr($c, 0, 8) == 'DIV_TAG_') $new_constants [$c] = $v;
        ...
        if (is_string($dialectTo)) $dialectTo = self::jsonDecode($dialectTo);
        if (is_object($dialectTo)) $dialectTo = get_object_vars($dialectTo);
        if (!is_array($dialectTo)) return false;

        foreach ($constants as $c => $v) {
            if (!isset ($dialectFrom [$c])) $dialectFrom [$c] = $v;
            if (!isset ($dialectTo [$c])) $dialectTo [$c] = $v;
        }
        ...
        foreach ($constants as $modifier_name => $v) {
            if (substr($modifier_name, 0, 17) == 'DIV_TAG_MODIFIER_') {
        ...
        $aggregate_functions = [];
        foreach ($constants as $c => $v) {
            if (substr($c, 0, 27) == 'DIV_TAG_AGGREGATE_FUNCTION_') $aggregate_functions[$dialectTo[$c]] = $dialectFrom[$c];
        }
            ...
            switch ($o) {
                ...
                case 'multi_modifiers' :
                    ...
                    while (true) {
                        ...
                        foreach ($parts as $k => $v) {
                            if (isset ($y_modifier [$v])) $parts [$k] = $y_modifier [$v];
                            if (isset ($y_modifier [$v . ':'])) $parts [$k] = $y_modifier [$v . ':'];
                        }
                ...
                case 'date_format' :
                    ...
                    while (true) {
                        ...
                        if ($po !== false) $sub_src = str_replace($dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER'], $dialectTo['DIV_TAG_VAR_MEMBER_DELIMITER'], substr($sub_src, 0, $po)) . $dialectTo['DIV_TAG_DATE_FORMAT_SEPARATOR'] . substr($sub_src, $po + $separator_len);
                ...
                case 'number_format' :
                    ...
                    while (true) {
                        ...
                        if ($po !== false) $sub_src = str_replace($dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER'], $dialectTo['DIV_TAG_VAR_MEMBER_DELIMITER'], substr($sub_src, 0, $po)) . $dialectTo['DIV_TAG_NUMBER_FORMAT_SEPARATOR'] . substr($sub_src, $po + $separator_len);
                    ...
                    break;
                case 'formulas' :
                    $src = $this->translateSimpleBlocks($src, $dialectFrom['DIV_TAG_FORMULA_BEGIN'], $dialectFrom['DIV_TAG_FORMULA_END'], $dialectTo['DIV_TAG_FORMULA_BEGIN'], $dialectTo['DIV_TAG_FORMULA_END'], $dialectFrom['DIV_TAG_FORMULA_FORMAT_SEPARATOR'], $dialectTo['DIV_TAG_FORMULA_FORMAT_SEPARATOR'], false);
                    break;
                case 'sub_parsers' :

                    foreach (self::$__sub_parsers as $sub_parser => $function) {
                        $src = str_replace($dialectFrom['DIV_TAG_SUBPARSER_BEGIN_PREFIX'] . $sub_parser . $dialectFrom['DIV_TAG_SUBPARSER_BEGIN_SUFFIX'], $dialectTo['DIV_TAG_SUBPARSER_BEGIN_PREFIX'] . $sub_parser . $dialectTo['DIV_TAG_SUBPARSER_BEGIN_SUFFIX'], $src);
                ...
                case 'html2txt' :
                    ...
                    while (true) {
                        ...
                        if ($po !== false) $sub_src = substr($sub_src, 0, $po) . $dialectTo['DIV_TAG_TXT_WIDTH_SEPARATOR'] . substr($sub_src, $po + $separator_len);
                ...
                case 'loops' :
                    ...
                    while (true) {
                        ...
                        $r = $this->getBlockRanges($src, $dialectFrom['DIV_TAG_LOOP_BEGIN_PREFIX'], $dialectFrom['DIV_TAG_LOOP_BEGIN_SUFFIX'], $dialectFrom['DIV_TAG_LOOP_END_PREFIX'], $dialectFrom['DIV_TAG_LOOP_END_SUFFIX'], $p, null, true, $dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER']);
                        ...
                        if ($po !== false) $sub_src = substr($sub_src, 0, $po) . $dialectTo['DIV_TAG_LOOP_VAR_SEPARATOR'] . substr($sub_src, $po + $separator_len);
                        ...
                        $src = substr($src, 0, $ini) . $dialectTo['DIV_TAG_LOOP_BEGIN_PREFIX'] . $key . $dialectTo['DIV_TAG_LOOP_BEGIN_SUFFIX'] . $sub_src . $dialectTo['DIV_TAG_LOOP_END_PREFIX'] . $key . $dialectTo['DIV_TAG_LOOP_END_SUFFIX'] . substr($src, $end + $suffix_len);
                ...
                case 'conditionals' :
                    ...
                    while (true) {
                        ...
                        $r = $this->getBlockRanges($src, $dialectFrom['DIV_TAG_CONDITIONAL_TRUE_BEGIN_PREFIX'], $dialectFrom['DIV_TAG_CONDITIONAL_TRUE_BEGIN_SUFFIX'], $dialectFrom['DIV_TAG_CONDITIONAL_TRUE_END_PREFIX'], $dialectFrom['DIV_TAG_CONDITIONAL_TRUE_END_SUFFIX'], $p, null, true, $dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER']);
                        ...
                        $key = str_replace($dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER'], $dialectTo['DIV_TAG_VAR_MEMBER_DELIMITER'], $key);
                        $src = substr($src, 0, $ini) . $dialectTo['DIV_TAG_CONDITIONAL_TRUE_BEGIN_PREFIX'] . str_replace($dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER'], $dialectTo['DIV_TAG_VAR_MEMBER_DELIMITER'], $key) . $dialectTo['DIV_TAG_CONDITIONAL_TRUE_BEGIN_SUFFIX'] . $sub_src . $dialectTo['DIV_TAG_CONDITIONAL_TRUE_END_PREFIX'] . $key . $dialectTo['DIV_TAG_CONDITIONAL_TRUE_END_SUFFIX'] . substr($src, $end + $suffix_len);
                        $p = $ini + 1;
                    }
                    ...
                    while (true) {
                        ...
                        $r = $this->getBlockRanges($src, $dialectFrom['DIV_TAG_CONDITIONAL_FALSE_BEGIN_PREFIX'], $dialectFrom['DIV_TAG_CONDITIONAL_FALSE_BEGIN_SUFFIX'], $dialectFrom['DIV_TAG_CONDITIONAL_FALSE_END_PREFIX'], $dialectFrom['DIV_TAG_CONDITIONAL_FALSE_END_SUFFIX'], $p, null, true, $dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER']);
                        ...
                        $sub_src = str_replace($dialectFrom['DIV_TAG_ELSE'], $dialectTo['DIV_TAG_ELSE'], $sub_src);
                        $key = str_replace($dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER'], $dialectTo['DIV_TAG_VAR_MEMBER_DELIMITER'], $key);
                        $src = substr($src, 0, $ini) . $dialectTo['DIV_TAG_CONDITIONAL_FALSE_BEGIN_PREFIX'] . $key . $dialectTo['DIV_TAG_CONDITIONAL_FALSE_BEGIN_SUFFIX'] . $sub_src . $dialectTo['DIV_TAG_CONDITIONAL_FALSE_END_PREFIX'] . $key . $dialectTo['DIV_TAG_CONDITIONAL_FALSE_END_SUFFIX'] . substr($src, $end + $suffix_len);
                        $p = $ini + 1;
                    }
                    break;
                ...
                case 'conditions' :
                    ...
                    while (true) {
                        ...
                        if ($po !== false) $sub_src = substr($sub_src, 0, $po) . $dialectTo['DIV_TAG_ELSE'] . substr($sub_src, $po + $else_tag_len);
                ...
                case 'tpl_vars' :
                    ...
                    while (true) {
                        ...
                        foreach ($missing_vars as $missing) $tpl_var_value = str_replace('$' . $missing, '$' . str_replace($dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER'], $dialectTo['DIV_TAG_VAR_MEMBER_DELIMITER'], $missing), $tpl_var_value);

                        $src = substr($src, 0, $ini) . $dialectTo['DIV_TAG_TPLVAR_BEGIN'] . str_replace($dialectFrom['DIV_TAG_TPLVAR_PROTECTOR'], $dialectTo['DIV_TAG_TPLVAR_PROTECTOR'], str_replace($dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER'], $dialectTo['DIV_TAG_VAR_MEMBER_DELIMITER'], substr($src, $ini + $begin_tag_len, $p1 - ($ini + $begin_tag_len)))) . $dialectTo['DIV_TAG_TPLVAR_ASSIGN_OPERATOR'] . $tpl_var_value . $dialectTo['DIV_TAG_TPLVAR_END'] . substr($src, $end + $end_tag_len);
                ...
                case 'default_replace' :
                    $src = $this->translateSimpleBlocks($src, $dialectFrom['DIV_TAG_DEFAULT_REPLACEMENT_BEGIN'], $dialectFrom['DIV_TAG_DEFAULT_REPLACEMENT_END'], $dialectTo['DIV_TAG_DEFAULT_REPLACEMENT_BEGIN'], $dialectTo['DIV_TAG_DEFAULT_REPLACEMENT_END']);
                    break;
                ...
                case 'preprocessed' :
                    $src = $this->translateSimpleBlocks($src, $dialectFrom['DIV_TAG_PREPROCESSED_BEGIN'], $dialectFrom['DIV_TAG_PREPROCESSED_END'], $dialectTo['DIV_TAG_PREPROCESSED_BEGIN'], $dialectTo['DIV_TAG_PREPROCESSED_END'], $dialectTo['DIV_TAG_PREPROCESSED_SEPARATOR']);
                    break;

                case 'capsules' :
                    $src = $this->translateKeyBlocks($src, $dialectFrom['DIV_TAG_CAPSULE_BEGIN_PREFIX'], $dialectFrom['DIV_TAG_CAPSULE_BEGIN_SUFFIX'], $dialectFrom['DIV_TAG_CAPSULE_END_PREFIX'], $dialectFrom['DIV_TAG_CAPSULE_END_SUFFIX'], $dialectTo['DIV_TAG_CAPSULE_BEGIN_PREFIX'], $dialectTo['DIV_TAG_CAPSULE_BEGIN_SUFFIX'], $dialectTo['DIV_TAG_CAPSULE_END_PREFIX'], $dialectTo['DIV_TAG_CAPSULE_END_SUFFIX'], $dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER'], $dialectTo['DIV_TAG_VAR_MEMBER_DELIMITER']);
                    break;

                case 'multi_replace' :
                    $src = $this->translateKeyBlocks($src, $dialectFrom['DIV_TAG_MULTI_REPLACEMENT_BEGIN_PREFIX'], $dialectFrom['DIV_TAG_MULTI_REPLACEMENT_BEGIN_SUFFIX'], $dialectFrom['DIV_TAG_MULTI_REPLACEMENT_END_PREFIX'], $dialectFrom['DIV_TAG_MULTI_REPLACEMENT_END_SUFFIX'], $dialectTo['DIV_TAG_MULTI_REPLACEMENT_BEGIN_PREFIX'], $dialectTo['DIV_TAG_MULTI_REPLACEMENT_BEGIN_SUFFIX'], $dialectTo['DIV_TAG_MULTI_REPLACEMENT_END_PREFIX'], $dialectTo['DIV_TAG_MULTI_REPLACEMENT_END_SUFFIX'], $dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER'], $dialectTo['DIV_TAG_VAR_MEMBER_DELIMITER']);
                    break;
                ...
                case 'location_content' :
                    $src = $this->translateKeyBlocks($src, $dialectFrom['DIV_TAG_LOCATION_CONTENT_BEGIN_PREFIX'], $dialectFrom['DIV_TAG_LOCATION_CONTENT_BEGIN_SUFFIX'], $dialectFrom['DIV_TAG_LOCATION_CONTENT_END_PREFIX'], $dialectFrom['DIV_TAG_LOCATION_CONTENT_END_SUFFIX'], $dialectTo['DIV_TAG_LOCATION_CONTENT_BEGIN_PREFIX'], $dialectTo['DIV_TAG_LOCATION_CONTENT_BEGIN_SUFFIX'], $dialectTo['DIV_TAG_LOCATION_CONTENT_END_PREFIX'], $dialectTo['DIV_TAG_LOCATION_CONTENT_END_SUFFIX'], $dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER'], $dialectTo['DIV_TAG_VAR_MEMBER_DELIMITER']);
                    break;
            }
        }
    ...
    {
        if (self::$__log_mode === true) $this->logger("Translating from some dialect to current dialect...");
    ...
    {
        if (self::$__log_mode === true) $this->logger("Translating from current dialect to some dialect...");
     ...
     * @return string
     */
    final private function subParse_parse($src, $items = null)
    {
        if (is_null($items)) $items = $this->__items;
    ...
    final private function subParse_html_wysiwyg($src)
    {
        ...
        $new_code = '';
        for ($i = 0; $i < $l; $i++) {
            if ($src [$i] != "\n" && $src [$i] != "\t" && $src [$i] != "\r") $new_code .= '&#' . ord($src [$i]) . ';';
            else
                $new_code .= $src [$i];
        }
    ...
    final private function subParse_join($src, $items)
    {
        ...
        $var_name = trim($arr[0]);
        $delimiter = "";
        if (isset($arr[1])) $delimiter = $arr[1];
     ...
     * @return string
     */
    final static function div($src = null, $items = null, $ignore = [], $min_level = 1, $discard_file_system = false)
    {
        $class = get_class();
     ...
     * @return string
     */
    final static function teaser($text, $max_length = 600)
    {
        $delimiter = strpos($text, DIV_TAG_TEASER_BREAK);
        if ($max_length == 0 && $delimiter === false) return $text;
        ...
        foreach ($break_points as $points) {
            foreach ($points as $point => $offset) {
                ...
                if ($reversed_pos !== false) $min_reversed_pos = min($reversed_pos + $offset, $min_reversed_pos);
            }
     ...
     * @return mixed
     */
    final static function jsonDecode($str, $items = [], &$missing_vars = [])
    {
        ...
        if (isset ($str [0])) {
            if ($str [0] == '$') {
                ...
                if (self::issetVar($str, $items)) $r = self::getVarValue($str, $items);
                else
                    $missing_vars [] = $str;
                ...
                if (is_numeric($str)) {
                    ...
                    for ($c = 0; $c < $str_len_chars; ++$c) {
                        ...
                        switch (true) {
                            ...
                            case $sub_str_chars_c_2 == '\\\\' :
                            case $sub_str_chars_c_2 == '\\/' :
                                if (($delimiter == '"' && $sub_str_chars_c_2 != '\\\'') || ($delimiter == "'" && $sub_str_chars_c_2 != '\\"')) $utf8 .= $chars{++$c};
                                break;
                    ...
                    if ($chars == '') if (reset($stk) == 3) return $arr;
                    else
                        return $obj;
                    ...
                    for ($c = 0; $c <= $str_len_chars; ++$c) {
                        ...
                        if (($c == $str_len_chars) || (($chars{$c} == ',') && ($top ['what'] == 1))) {
                            ...
                            array_pop($stk);
                        } elseif (($chars{$c} == '[') && in_array($top ['what'], [
                                1,
                                3,
                                4
                            ])) {
                            ...
                            array_pop($stk);
                        } elseif (($sub_str_chars_c_2 == '/*') && in_array($top ['what'], [
                                1,
                                3,
                                4
                            ])) {
    ...
    final static function jsonEncode($data)
    {
        if (is_array($data) || is_object($data)) {
            ...
            if ($is_list) $json = '[' . implode(',', array_map('div::jsonEncode', $data)) . ']';
            else {
                $items = [];
     ...
     * @return boolean
     */
    final static function atLeastOneString($haystack, $needles = [])
    {
    ...
    final static function varExists($var, &$items = null)
    {
        ...
        if (count($sub_vars) === 1) {
            ...
            if ($l + 1 < strlen($var)) {
                if (is_array($items)) return self::varExists(substr($var, $l + 1), $items[$temp_sub_var]);
                if (is_object($items)) return self::varExists(substr($var, $l + 1), $items->$temp_sub_var);
            }
        }
    ...
    {
        if (substr(strtolower($filename), 0, 7) == 'http://' || substr(strtolower($filename), 0, 8) == 'https://' || substr(strtolower($filename), 0, 6) == 'ftp://') return false;

        if (strlen($filename) > DIV_MAX_FILENAME_SIZE) return false;

        if (file_exists($filename)) if (is_file($filename)) return true;
     ...
     * @return boolean
     */
    final static function isDir($dir_name)
    {
        if (substr(strtolower($dir_name), 0, 7) == 'http://' || substr(strtolower($dir_name), 0, 8) == 'https://' || substr(strtolower($dir_name), 0, 6) == 'ftp://') return false;

        if (strlen($dir_name) > DIV_MAX_FILENAME_SIZE) return false;
    ...
    {
        if (substr(strtolower($filename), 0, 7) == 'http://' || substr(strtolower($filename), 0, 8) == 'https://' || substr(strtolower($filename), 0, 6) == 'ftp://') return $filename;
        ...
        foreach ($include_paths as $include_path) {
            ...
            if (file_exists($path)) if (is_file($path)) return file_get_contents($path);
        }
        ...
        if (is_array($mixed)) {
            if (self::isArrayOfArray($mixed) === true) {
                ...
                foreach ($mixed as $key => $value) {
                    $t = "";
                    if (!is_numeric($key) && trim("$key") != "" && $key != null) $t = "$key: <br>";
            ...
            if (is_object($mixed)) {
                ...
                if (is_bool($mixed)) $html = ($mixed === true ? "TRUE" : "FALSE");
                else {
     ...
     * @return boolean
     */
    final static function isValidVarName($var_name)
    {
    ...
    final static function isValidPHPCode(
        $code, $multi_lines = true, $valid_tokens = null, $invalid_vars = [
        '$this',
        '$_SESSION',
        '$GLOBALS',
        '$_POST',
        ...
        '$_ENV',
        '$_REQUEST'
    ], $allow_classes = false, $allow_functions = false
    )
    {
        ...
        foreach ($t as $key => $value) {
            if (is_array($value)) $t [$key] [0] = token_name($value [0]);
        }
            ...
            if (is_array($token)) {
                ...
                switch ($n) {
                    ...
                    case 'T_STRING' :
                        ...
                        if ($last_token == 'T_CLASS') $class_name = true;
                        if ($last_token == 'T_FUNCTION') $function_name = true;
    ...
    static function internalMsg($msg, $category = 'global')
    {
        ...
        if (isset ($d ['class'])) $caller = $d ['class'] . "::" . $caller;
     ...
     * @param string $level
     */
    static function error($err_msg, $level = DIV_ERROR_WARNING)
    {
        self::$__errors [] = [
            $err_msg,
            $level
        ...
        ob_start();
        if ($is_cli) $err_msg = self::htmlToText($err_msg, null);

        if ($is_cli === false) echo "<div style = \"z-index:9999; position: absolute; top: " . ((count(self::$__errors) - 1) * 50 + 10) . "px; right: 20px; width: 600px;max-height: 600px; overflow:auto;; font-family: courier; padding: 10px;";

        switch ($level) {
            case DIV_ERROR_WARNING :
                if (!$is_cli) echo "background: yellow; border: 1px solid black; color: black;\">[$level] $err_msg</div>";
                else
                    echo "$level: $err_msg\n";
                break;
            case DIV_ERROR_FATAL :
                if (!$is_cli) echo "background: red; border: 1px solid black; color: white;\">[$level] $err_msg</div>";
                else
                    echo "$level: $err_msg\n";
                break;
        }
        ...
        if ($is_cli) echo '[[]]' . self::htmlToText($msg, null) . "\n";
        else
            echo $msg;
