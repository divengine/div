...
class div
{
  ...
  public function __construct($src = null, $items = null, $ignore = [])
  {
    ...
    if (!key_exists('div', $this->__items)) {
      $this->__items['div'] = [];
    }
  ...
  final static function createAuxiliaryEngine(&$from = null)
  {
    ...
    if (self::$__engine !== null) {
      if (get_class(self::$__engine) != $class_name) {
        self::$__engine = null;
      }
    }
  ...
  final public function saveOperation($params = [])
  {
    ...
    if (!key_exists($this->__crc, self::$__remember)) {
      self::$__remember[$this->__crc] = [];
    }
    if (!key_exists($id, self::$__remember[$this->__crc])) {
  ...
  {
    if (key_exists($var, self::$__system_data_allowed)) {
  ...
  final static function getSystemData()
  {
    ...
    if (self::$__system_data == null) {
      ...
      if (key_exists("div{$d}ascii", self::$__system_data_allowed)) {
        $ascii = [];
        for ($i = 0; $i <= 255; $i++) {
          $ascii [$i] = chr($i);
        }
      ...
      if (key_exists("div{$d}now", self::$__system_data_allowed)) {
        self::$__system_data ["div{$d}now"] = time();
      }
      if (key_exists("div{$d}post", self::$__system_data_allowed)) {
        self::$__system_data ["div{$d}post"] = $_POST;
      }
      if (key_exists("div{$d}get", self::$__system_data_allowed)) {
        self::$__system_data ["div{$d}get"] = $_GET;
      }
      if (key_exists("div{$d}server", self::$__system_data_allowed)) {
        self::$__system_data ["div{$d}server"] = $_SERVER;
      }
      if (key_exists("div{$d}session", self::$__system_data_allowed)) {
        self::$__system_data ["div{$d}session"] = isset($_SESSION) ? $_SESSION : [];
      }
      if (key_exists("div{$d}version", self::$__system_data_allowed)) {
        self::$__system_data ["div{$d}version"] = self::$__version;
      }
      if (key_exists("div{$d}script_name", self::$__system_data_allowed)) {
  ...
  {
    $id = serialize($search);
    if (!key_exists($var, self::$__defaults_by_var)) {
      self::$__defaults_by_var [$var] = [];
    }
    if (!key_exists($id, self::$__defaults_by_var [$var]) && $update === true) {
  ...
  final static function repairSubParsers()
  {
    ...
    foreach (self::$__sub_parsers as $parser => $function) {
      ...
      foreach ($events as $event) {
        if (!key_exists("$parser:$event", self::$__sub_parsers)) {
  ...
  {
    $id = serialize($search);
    if (key_exists($id, self::$__defaults)) {
  ...
  final static function delDefaultByVar($var, $search)
  {
    if (key_exists($var, self::$__defaults_by_var)) {
      $id = serialize($search);
      if (key_exists($id, self::$__defaults_by_var[$var])) {
  ...
  final public function isLiteral($key)
  {
    ...
    return key_exists($key, $literals);
  }
      ...
      if ($p2 > $p1) {
        ...
        if (!empty($value)) {
          ...
          foreach ($value as $kk => $item) {
            ...
            if ($go_is_last) {
              $anothers ['_is_last'] = ($ii == $keys_count);
            }
            if ($go_list) {
              $anothers ['_list'] = $key;
            }
          ...
          foreach ($xitems as $xkey => $item) {
            ...
            foreach ($item as $kkk => $vvv) {
              if (key_exists($kkk, self::$__globals_design)) {
            ...
            foreach ($item as $kkk => $vvv) {
              if (key_exists($kkk, $engine->__memory)) {
            ...
            foreach ($item as $kkk => $vvv) {
              if (key_exists($kkk, $this->__memory)) {
  ...
  final public function parseInclude(&$items)
  {
    ...
    if (is_array($items)) {
      foreach ($items as $key => $value) {
        if (key_exists($key, $this->__ignore) && $this->__ignore[$key] === true) {
          continue;
        }
                  ...
                  if (!is_numeric($from) && !is_numeric($to)) {
                    if (!empty ($from) && !empty ($to)) {
                      ...
                      foreach ($extracts as $extract) {
                        ...
                        if (isset($data_path ['offset'])) {
                          if ($i < $data_path ['offset'] * 1) {
                            continue;
                          }
                        }
                        ...
                        if (isset($data_path ['limit'])) {
                          if ($i == $data_path ['limit'] * 1) {
                            break;
                          }
                        }
                      }
  ...
  final public function parsePreprocessed($items)
  {
    ...
    while (true) {
      ...
      $lr = $this->getListRanges(null, false);
    ...
    while (true) {
      ...
      if ($ni !== false) {
        ...
        if (is_array($temp_value)) {
          if (!key_exists($var, self::$__globals_design)) {
            $setup_design = false;
          }
      ...
      if ($setup_design) {
        ...
        do {
          ...
          if ($ni !== false) {
            ...
            if (!key_exists($nv, self::$__globals_design) && self::issetVar($nv, $items)) {
              $setup_design = false;
              break;
            }
          }
      ...
      // Check protection
      $ke = key_exists($var, self::$__globals_design);
      if ((!$ke || ($ke && !key_exists($var, self::$__globals_design_protected)))) {
        $exp = trim($exp);
        ...
        elseif (substr($exp, 0, 1) == "$") {
          ...
          if (!self::issetVar($var, $items) || self::issetVar($var, self::$__globals_design)) {
            ...
            if (($_exp != '{' && $_exp != "[" && !is_numeric($_exp) && $_exp != '"' && $_exp != "'") || (substr($exp, 0, strlen(DIV_TAG_INCLUDE_BEGIN)) == DIV_TAG_INCLUDE_BEGIN && substr($exp, 0 - strlen(DIV_TAG_INCLUDE_END)) == DIV_TAG_INCLUDE_END)
              || (substr($exp, 0, strlen(DIV_TAG_PREPROCESSED_BEGIN)) == DIV_TAG_PREPROCESSED_BEGIN && substr($exp, 0 - strlen(DIV_TAG_PREPROCESSED_END)) == DIV_TAG_PREPROCESSED_END)) {
              $exp = '"' . str_replace('"', '\"', $exp) . '"';
            }
  ...
  final public function parseNumberFormat(&$items = [])
  {
    ...
    foreach ($ranges as $range) {
      ...
      if (!key_exists($arr[0], $items) && is_numeric($arr[0])) {
                  ...
                  if (key_exists($var, $v)) {
                    ...
                    switch ($function) {
                      case DIV_TAG_AGGREGATE_FUNCTION_MIN :
                        if ($min * 1 > $v [$var] * 1 || $min === null) {
                          $min = $v [$var] * 1;
                        }
                        break;
                      case DIV_TAG_AGGREGATE_FUNCTION_MAX :
                        if ($max * 1 < $v [$var] * 1 || $max === null) {
                          $max = $v [$var] * 1;
                        }
                        break;
  ...
  final public function parseCapsules(&$items = [])
  {
    ...
    while (true) {
      ...
      if (is_array($this->__items_orig)) {
        if (key_exists($key, $this->__items_orig)) {
  ...
  final private function memory(&$items)
  {
    ...
    foreach ($vars as $var) {
      if (!key_exists($var, $items) || strpos($var, DIV_TAG_VAR_MEMBER_DELIMITER) !== false) {
    ...
    while (true) {
      ...
      if (self::isValidMacro($this->__temp ['code'])) {
        ...
        foreach ($this->__temp ['vars'] as $var => $value) {
          ...
          if (!key_exists($var, $items) || key_exists($var, self::$__globals_design)) {
    ...
    foreach (self::$__remember [$checksum] as $params) {
      ...
      switch ($params ['o']) {
        ...
        case 'replace_sub_match_wordwrap' :
          ...
          $value = self::anyToStr($value);
          $this->__src =
            str_replace($simple . $params ['key'] . DIV_TAG_SUBMATCH_SEPARATOR . $params ['param'] . DIV_TAG_REPLACEMENT_SUFFIX, $vpx . wordwrap("{$value}", intval(substr($params ['param'], strlen(DIV_TAG_MODIFIER_WORDWRAP))), "\n", 1) . $vsx,
              $this->__src);
          break;
  ...
  final public function prepareDialect($src = null, $properties = null, $update = true)
  {
    ...
    if (key_exists('DIALECT', $properties)) {
  ...
  final public function parse($from_original = true, $item_index = null, $min_level = 1)
  {
    ...
    if (trim($this->__src) != "") {
      ...
      // Reserved vars
      if (!key_exists('_empty', $items)) {
        $items ['_empty'] = [];
      }
      if (!key_exists('_', $items)) {
        $items ['_'] = [];
      }
   ...
   * @return string
   */
  final private function translateKeyBlocks(
    $src,
    $begin_prefix,
    $begin_suffix,
    $end_prefix,
  ...
  final public function translate($dialectFrom = [], $dialectTo = [], $src = null, $items = null)
  {
    ...
    $aggregate_functions = [];
    foreach ($constants as $c => $v) {
      if (substr($c, 0, 27) == 'DIV_TAG_AGGREGATE_FUNCTION_') {
    ...
    foreach ($order as $o => $priority) {
      switch ($o) {
        ...
        case 'number_format' :
          ...
          break;
        case 'formulas' :
          $src = $this->translateSimpleBlocks($src, $dialectFrom['DIV_TAG_FORMULA_BEGIN'], $dialectFrom['DIV_TAG_FORMULA_END'], $dialectTo['DIV_TAG_FORMULA_BEGIN'], $dialectTo['DIV_TAG_FORMULA_END'], $dialectFrom['DIV_TAG_FORMULA_FORMAT_SEPARATOR'],
        ...
        case 'conditionals' :
          ...
          while (true) {
            ...
            $key = str_replace($dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER'], $dialectTo['DIV_TAG_VAR_MEMBER_DELIMITER'], $key);
            $src =
              substr($src, 0, $ini) . $dialectTo['DIV_TAG_CONDITIONAL_TRUE_BEGIN_PREFIX'] . str_replace($dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER'], $dialectTo['DIV_TAG_VAR_MEMBER_DELIMITER'], $key) . $dialectTo['DIV_TAG_CONDITIONAL_TRUE_BEGIN_SUFFIX']
        ...
        case 'tpl_vars' :
          ...
          while (true) {
            ...
            $src = substr($src, 0, $ini) . $dialectTo['DIV_TAG_TPLVAR_BEGIN'] . str_replace($dialectFrom['DIV_TAG_TPLVAR_PROTECTOR'], $dialectTo['DIV_TAG_TPLVAR_PROTECTOR'],
                str_replace($dialectFrom['DIV_TAG_VAR_MEMBER_DELIMITER'], $dialectTo['DIV_TAG_VAR_MEMBER_DELIMITER'], substr($src, $ini + $begin_tag_len, $p1 - ($ini + $begin_tag_len)))) . $dialectTo['DIV_TAG_TPLVAR_ASSIGN_OPERATOR'] . $tpl_var_value
  ...
  {
    if (substr(strtolower($dir_name), 0, 7) == 'http://' || substr(strtolower($dir_name), 0, 8) == 'https://' || substr(strtolower($dir_name), 0, 6) == 'ftp://') {
      return false;
    }
