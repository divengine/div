 ...
 * @link http://github.com/rrodriguezr/div
 * @example example.php
 * @example example.tpl
 */
...
define('DIV_PHP_VALID_TOKENS_FOR_EXPRESSIONS',
    'T_ARRAY,T_ARRAY_CAST,T_BOOLEAN_AND,T_BOOLEAN_OR,T_BOOL_CAST,T_CHARACTER,T_CONSTANT_ENCAPSED_STRING,' .
...
define('DIV_PHP_VALID_TOKENS_FOR_MACROS',
    'T_AS,T_DO,T_DOUBLE_COLON,T_ECHO,T_ELSE,T_ELSEIF,T_FOR,T_FOREACH,T_IF,T_MOD_EQUAL,T_MUL_EQUAL,' .
...
define('DIV_PHP_ALLOWED_METHODS',
    'getRanges,asThis,atLeastOneString,getLastKeyOfArray,' .
...
class div
{
    ...
    public function __construct($src = null, $items = null, $ignore = array())
    {
        ...
        if (is_null(self::$__allowed_methods)) {
            ...
            if (self::$__super_class != $classname) {
                ...
                if (isset($keys[0]))
                    self::$__allowed_methods = array_merge(self::$__allowed_methods, array_combine($keys,
                        $keys));
            }
        }
    ...
    final static function getOperatingSystem()
    {
        if (isset($_SERVER['SERVER_SOFTWARE'])) {
            if (isset($_SERVER['WINDIR']) || strpos($_SERVER['SERVER_SOFTWARE'], 'Win32')
                !== false)
                return "win32";
            if (!isset($_SERVER['WINDIR']) && strpos($_SERVER['SERVER_SOFTWARE'], 'Linux')
                !== false)
                return "linux";
        }
     ...
     * @return array
     */
    final public function getRanges($tagini, $tagend, $src = null, $onlyfirst = false,
        $pos = 0)
    {
        $ranges = array();
        if (is_null($src))
                            ...
                            if (isset($this)) {
                                foreach ($this->__items as $key => $value)
                                    if (strpos($tagini, $key) !== false) {
                                        $this->logger('Unclosed tag ' . $tagini . ' at ' . $ini . ' character',
                                            DIV_ERROR_WARNING);
                                        break;
                                    }
                            }
     ...
     * @return array
     */
    final public function getBlockRanges($src = null, $begin_prefix = '{', $begin_suffix =
        '}', $end_prefix = '{/', $end_suffix = '}', $after = 0, $before = null, $onlyfirst = false,
        $var_member_delimiter = DIV_TAG_VAR_MEMBER_DELIMITER)
    {
        if (is_null($src))
            $src = $this->__src;
        if (!is_null($before))
    ...
    {
        $rangs = $this->getBlockRanges(null, DIV_TAG_LOOP_BEGIN_PREFIX,
            DIV_TAG_LOOP_BEGIN_SUFFIX, DIV_TAG_LOOP_END_PREFIX, DIV_TAG_LOOP_END_SUFFIX);
    ...
    final public function searchInCapsuleRanges(&$items = null, $pos = 0)
    {
        ...
        foreach ($items as $key => $value) {
            $rangs = $this->getRanges(DIV_TAG_CAPSULE_BEGIN_PREFIX . $key,
                DIV_TAG_CAPSULE_END_PREFIX . $key);
            foreach ($rangs as $rang)
                if ($pos >= $rang[0] && $pos <= $rang[1])
                    return true;
        }
    ...
    {
        $else = false;
        $range_conditions = $this->getRanges(DIV_TAG_CONDITIONS_BEGIN_PREFIX,
            DIV_TAG_CONDITIONS_END, $subsrc);
    ...
    {
        $else = false;
        $rangesx = $this->getRanges(DIV_TAG_LOOP_BEGIN_PREFIX, DIV_TAG_LOOP_END_PREFIX,
            $subsrc);
    ...
    final public function parseConditionalBlock($key, $value)
    {
        ...
        foreach ($passes as $flag) {
            ...
            if ($flag === false) {
                $tag_begin = DIV_TAG_CONDITIONAL_TRUE_BEGIN_PREFIX . $key .
                    DIV_TAG_CONDITIONAL_TRUE_BEGIN_SUFFIX;
                $tag_end = DIV_TAG_CONDITIONAL_TRUE_END_PREFIX . $key .
                    DIV_TAG_CONDITIONAL_TRUE_END_SUFFIX;
            } else {
                $tag_begin = DIV_TAG_CONDITIONAL_FALSE_BEGIN_PREFIX . $key .
                    DIV_TAG_CONDITIONAL_FALSE_BEGIN_SUFFIX;
                $tag_end = DIV_TAG_CONDITIONAL_FALSE_END_PREFIX . $key .
            ...
            while (true) {
                ...
                if (count($ranges) > 0) {
                    ...
                    if (array_search($key, array(
                        ...
                        "_is_odd",
                        "_id_even")) !== false) {
                        if ($this->isBlockInsideBlock(DIV_TAG_LOOP_BEGIN_PREFIX, DIV_TAG_LOOP_END_PREFIX,
                            $ini, $fin)) {
                            $pos = $fin + 1;
                            if (self::$__log_mode)
    ...
    final public function numberFormat($key, $value)
    {
        ...
        $tag_begin = DIV_TAG_NUMBER_FORMAT_PREFIX . $key .
        ...
        while (true) {
            ...
            if (!is_numeric($value)) {
                $this->__src = substr($this->__src, 0, $ini) . DIV_TAG_NUMBER_FORMAT_PREFIX . $value .
                    DIV_TAG_NUMBER_FORMAT_SEPARATOR . $format . DIV_TAG_NUMBER_FORMAT_SUFFIX .
            ...
            $this->__src = substr($this->__src, 0, $ini) . number_format($value, $decimals,
                $separator, $miles_sep) . substr($this->__src, $fin + $l2);
        }
    }
    ...
    final public function parseSubmatch(&$items, $key, $value, $modifiers = array())
    {
        ...
        if (strpos($this->__src, $key . DIV_TAG_SUBMATCH_SEPARATOR) !== false)
            foreach ($modifiers as $modifier)
                while (true) {
                    $tag_begin = DIV_TAG_REPLACEMENT_PREFIX . $modifier . $key .
                    ...
                    if (!is_numeric($arr[0]) || !is_numeric($arr[1])) {
                        if (substr("{$arr[1]}", 0, 1) == DIV_TAG_MODIFIER_TRUNCATE) {
                            ...
                            $this->saveOperation(array(
                                ...
                                "modifier" => $modifier,
                                "param" => $r));
                        } elseif (substr("{$arr[1]}", 0, 1) == DIV_TAG_MODIFIER_FORMAT ||
                    ...
                    $this->__src = substr($this->__src, 0, $ranges[0][0]) .
                        DIV_TAG_REPLACEMENT_PREFIX . "{$modifier}$nkey" . DIV_TAG_REPLACEMENT_SUFFIX . $right;
                }
    ...
    final public function parseFriendly()
    {
        ...
        while (true) {
            ...
            $ini = $r[0][0];
            $fin = $r[0][1];
            $this->__src = substr($this->__src, 0, $ini) . substr($this->__src, $ini + $l1,
                $fin - ($ini + $l1)) . substr($this->__src, $fin + $l2);
        }
    }
    ...
    final public function addLiteral($var)
    {
        ...
        $literals = self::getVarValue('div' . DIV_TAG_VAR_MEMBER_DELIMITER . 'literals',
        ...
        self::setVarValue('div' . DIV_TAG_VAR_MEMBER_DELIMITER . 'literals', $literals,
            self::$__globals_design, true);
    }
    ...
    {
        $val = self::getVarValue('div' . DIV_TAG_VAR_MEMBER_DELIMITER . 'literals', $this->
            __memory);
    ...
    final public function parseMatch($key, $value, &$engine, $ignore_logical_order = false)
    {
        ...
        if ($is_string || is_numeric($value)) {
            ...
            if (strpos($value, $prefix . DIV_TAG_MODIFIER_SIMPLE . $key . $suffix) !== false) {
                ...
                self::error("Was detected an infinite loop in recursive replacement of ${$key}.",
                    DIV_ERROR_WARNING);
            }
            ...
            if ($px !== false) {
                ...
                $substr = str_replace($prefix . $mod . $key . $suffix, $vpx . $value . $vsx, $substr,
                    $rcount); // simple replacement
                ...
                if (strpos($substr, $prefix . $mod) !== false) {
                    $substr = str_replace(DIV_TAG_REPLACEMENT_PREFIX . $mod . $key . $suffix, $vpx .
                        ucfirst($value) . $vsx, $substr, $rcount);
                    if ($rcount > 0 && !isset(self::$__dont_remember_it[$key]))
                        $this->saveOperation(array(
                            "o" => 'simple_replacement',
                            'key' => $key,
                ...
                if (strpos($substr, $prefix . $mod) !== false) {
                    $substr = str_replace($prefix . $mod . $key . $suffix, $vpx . ucwords($value) .
                        $vsx, $substr, $rcount);
                    if ($rcount > 0 && !isset(self::$__dont_remember_it[$key]))
                        $this->saveOperation(array(
                            'o' => 'simple_replacement',
                ...
                if (strpos($substr, $prefix . $mod) !== false) {
                    $substr = str_replace($prefix . $mod . $key . $suffix, $vpx . strtoupper($value) .
                        $vsx, $substr, $rcount);
                    if ($rcount > 0 && !isset(self::$__dont_remember_it[$key]))
                        $this->saveOperation(array(
                            'o' => 'simple_replacement',
                ...
                if (strpos($substr, $prefix . $mod) !== false) {
                    $substr = str_replace($prefix . $mod . $key . $suffix, strlen($value), $substr,
                        $rcount);
                    if ($rcount > 0 && !isset(self::$__dont_remember_it[$key]))
                        $this->saveOperation(array(
                            'o' => 'simple_replacement',
                ...
                if (strpos($substr, $prefix . $mod) !== false) {
                    $substr = str_replace($prefix . $mod . $key . $suffix, self::getCountOfWords($value),
                        $substr, $rcount);
                    if ($rcount > 0 && !isset(self::$__dont_remember_it[$key]))
                        $this->saveOperation(array(
                            'o' => 'simple_replacement',
                ...
                if (strpos($substr, $prefix . $mod) !== false) {
                    $substr = str_replace($prefix . $mod . $key . $suffix, self::
                        getCountOfSentences($value), $substr, $rcount);
                    if ($rcount > 0 && !isset(self::$__dont_remember_it[$key]))
                        $this->saveOperation(array(
                            'o' => 'simple_replacement',
                            'key' => $key,
                ...
                if (strpos($substr, $prefix . $mod) !== false) {
                    $substr = str_replace($prefix . $mod . $key . $suffix, self::
                        getCountOfParagraphs($value), $substr, $rcount);
                    if ($rcount > 0 && !isset(self::$__dont_remember_it[$key]))
                        $this->saveOperation(array(
                            'o' => 'simple_replacement',
                            'key' => $key,
                ...
                if (strpos($substr, $prefix . $mod) !== false) {
                    $substr = str_replace($prefix . $mod . $key . $suffix, $vpx . urlencode($value) .
                        $vsx, $substr, $rcount);
                    if ($rcount > 0 && !isset(self::$__dont_remember_it[$key]))
                        $this->saveOperation(array(
                            'o' => 'simple_replacement',
                ...
                if (strpos($substr, $prefix . $mod) !== false) {
                    $substr = str_replace($prefix . $mod . $key . $suffix, $vpx . rawurlencode($value) .
                        $vsx, $substr, $rcount);
                    if ($rcount > 0 && !isset(self::$__dont_remember_it[$key]))
                        $this->saveOperation(array(
                            'o' => 'simple_replacement',
                ...
                foreach (self::$__custom_modifiers as $modifier) {
                    ...
                    if ($proced)
                        if (strpos($substr, $modifier[0]) !== false) {
                            ...
                            $substr = str_replace($prefix . $modifier[0] . $key . $suffix, $vpx . $vvalue .
                                $vsx, $substr);
                        }
                }
        ...
        if (strpos($this->__src, $prefix . $mod . $key . $suffix) !== false) {
            $this->__src = str_replace($prefix . $mod . $key . $suffix, $vpx . self::
                jsonEncode($value) . $vsx, $this->__src, $rcount);
            if ($rcount > 0 && !isset(self::$__dont_remember_it[$key]))
                $this->saveOperation(array(
                    'o' => 'json_encode',
                    'key' => $key,
    ...
    final public function parseIterations(&$items, &$src = null)
    {
        ...
        while (true) {
            $ranges = $this->getRanges(DIV_TAG_ITERATION_BEGIN_PREFIX, DIV_TAG_ITERATION_END,
                $src, true);
            ...
            if (is_numeric($range[0]) && is_numeric($range[1])) {
                ...
                $src = substr($src, 0, $p) . DIV_TAG_LOOP_BEGIN_PREFIX . $key .
                    DIV_TAG_LOOP_BEGIN_SUFFIX . $sitervar . $subsrc . DIV_TAG_LOOP_END_PREFIX . $key .
                    DIV_TAG_LOOP_END_SUFFIX . substr($src, $p2 + $l2);
    ...
    final public function parseListBlock($value, $key, $items)
    {
        ...
        while (true) {
            ...
            if ($p2 > $p1) {
                ...
                if ($empty !== false) {
                    $body_parts = array(substr($minihtml, 0, $empty), substr($minihtml, $empty +
                            strlen(DIV_TAG_EMPTY)));
                } else
                    $body_parts = array($minihtml, "");
    ...
    final public function parseList($items = null, $superkey = "")
    {
        ...
        if (strpos($this->__src, DIV_TAG_LOOP_BEGIN_PREFIX . $superkey) !== false)
            foreach ($items as $key => $value) {
                $key = $superkey . $key;
                if (strpos($this->__src, DIV_TAG_LOOP_BEGIN_PREFIX . $key .
                    DIV_TAG_VAR_MEMBER_DELIMITER) !== false) {
                    if (!is_array($value))
                        if (is_object($value)) {
        ...
        $pos = array();
        foreach ($items as $key => $value) {
            $p = strpos($this->__src, DIV_TAG_LOOP_BEGIN_PREFIX . $superkey . $key .
    ...
    final public function parseIgnore()
    {
        ...
        for ($i = 0; $i < 2; $i++) {
            ...
            while (true) {
                ...
                if (self::searchInRanges($this->getRanges(DIV_TAG_COMMENT_BEGIN,
                    DIV_TAG_COMMENT_END), $ranges[0][0]) !== false) {
                    $pos = $ranges[0][1] + 1;
                    continue;
                }

                $id = uniqid();
                self::$__ignored_parts[$id] = substr($this->__src, $ranges[0][0] + $l1, $ranges[0][1] -
                    $ranges[0][0] - $l1);
                $this->__src = substr($this->__src, 0, $ranges[0][0]) . '{' . $id . '}' . substr($this->
                    __src, $ranges[0][1] + $l2);
                $pos = $ranges[0][0] + 1;
            }
        }
    ...
    final public function getTplPath($path)
    {
        ...
        $path = str_replace("." . DIV_DEFAULT_TPL_FILE_EXT . "." .
            DIV_DEFAULT_TPL_FILE_EXT, "." . DIV_DEFAULT_TPL_FILE_EXT, $path);
        ...
        if (is_null($return)) {
            ...
            foreach (self::$__includes_historial as $ih) {
                ...
                if ((self::fileExists($fullpath) || self::fileExists($fullpath)) && $similar >=
                    $max)
                    $return = $fullpath;
            }
        }
    ...
    final public function parseInclude(&$items)
    {
        ...
        if (is_array($items))
            foreach ($items as $key => $value) {
                ...
                if (strpos($this->__src, $prefix . $key . $suffix) !== false && self::isString($value)) {
                    ...
                    $this->__src = str_replace($prefix . $key . $suffix, $prefix . $value . $suffix,
                        $this->__src);
                }
            }
        ...
        while (true) {
            ...
            if ($path != $this->__path && !self::detectRecursiveInclusion($exclusion, $path,
                $ini)) {
                if (self::fileExists($path)) {
                    $c = "";
                    if (!is_dir($path)) {
                        ...
                        if (self::$__docs_on) {
                            if (self::fileExists($this->__path) || self::fileExists(PACKAGES . $this->
                                __path)) {
                    ...
                    $this->__src = substr($this->__src, 0, $ini) . $c . substr($this->__src, $fin +
                        $l2);
                } else {
                    $id = uniqid();
                    $restore[$id] = substr($this->__src, $ini, $fin + $l2 - $ini);
                    $this->__src = substr($this->__src, 0, $ini) . $id . substr($this->__src, $fin +
                        $l2);
                }
            } else {
                if (trim($path) != '') {
                    self::error("Recursive inclusion of template '$path' in '" . substr($this->
                        __src, $ini - 20, 20) . "'is not allowed", DIV_ERROR_WARNING);
                }
    ...
    final public function parsePreprocessed($items)
    {
        ...
        while (true) {
            ...
            if (self::fileExists($path)) {
                ...
                $this->__src = substr($this->__src, 0, $ini) . $pre . substr($this->__src, $fin +
                    $l2);
            } else {
                $id = uniqid();
                $restore[$id] = substr($this->__src, $ini, $fin + $l2 - $ini);
    ...
    final public function parseComments($section = null)
    {
        ...
        while (true) {
            ...
            if (self::searchInRanges($this->getRanges(DIV_TAG_INCLUDE_BEGIN,
                DIV_TAG_INCLUDE_END), $ranges[0][0]) !== false) {
                $pos = $ranges[0][1] + 1;
                continue;
            }
            ...
            if (self::$__docs_on) {
                $subsrc = substr($this->__src, $ranges[0][0] + $lbegin, $ranges[0][1] - $ranges[0][0] -
                    $lbegin);
                $arr = explode("\n", $subsrc);
                            ...
                            if (isset(self::$__docs[$section][$prop])) {
                                if (!is_array(self::$__docs[$section][$prop])) {
                                    if (trim(self::$__docs[$section][$prop]) !== '')
                                        self::$__docs[$section][$prop] = array(self::$__docs[$section][$prop]);
                                }
                                if (isset(self::$__docs[$section][$prop][0]) || (!isset(self::$__docs[$section][$prop][0]) &&
                                    trim($value) !== ''))
            ...
            // Extract
            $this->__src = substr($this->__src, 0, $ranges[0][0]) . substr($this->__src, $ranges[0][1] +
                strlen(DIV_TAG_COMMENT_END));
        }
    }
        ...
        if (is_null($current)) {
            ...
            foreach ($parts as $part) {
                if (isset($itemsx[$s . $part])) {
                    $current = self::getVarValue(substr($var, strlen($s . $part) + 1), $itemsx[$s .
                        $part]);
                    if (!is_null($current))
                        break;
                }
    ...
    final public function parseData(&$items)
    {
        ...
        while (true) {
            ...
            if ($this->searchInRanges($this->getRanges(DIV_TAG_CONDITIONS_BEGIN_PREFIX,
                DIV_TAG_CONDITIONS_END), $ini)) {
                $pos = $ini + 1;
                continue;
            }
            ...
            $var = str_replace(array("[", "]"), array(DIV_TAG_VAR_MEMBER_DELIMITER, ""), $var);
            ...
            // Check protection
            if ((!isset(self::$__globals_design[$var]) || (isset(self::$__globals_design[$var]) &&
                !isset(self::$__globals_design_protected[$var])))) {
                ...
                if (substr($exp, 0, 2) == "->") { // parsing a method
                    ...
                    $setup = $value != DIV_METHOD_NOT_EXISTS && (!self::issetVar($var, $items) ||
                        self::issetVar($var, self::$__globals_design));
                    ...
                    if (!self::issetVar($var, $items) || self::issetVar($var, self::$__globals_design)) {
                        ...
                        if (self::fileExists($exp) && !self::isDir($exp)) {
                            ...
                            if ($fgc != "")
                                $exp = $fgc;
                        } elseif (self::fileExists($exp . "." . DIV_DEFAULT_DATA_FILE_EXT) && !self::
                        isDir($exp . "." . DIV_DEFAULT_DATA_FILE_EXT)) {
                            $fgc = self::getFileContents($exp . "." . DIV_DEFAULT_DATA_FILE_EXT);
                            if ($fgc != "")
                                $exp = $fgc;
                        } elseif (self::fileExists($exp . "." . DIV_DEFAULT_TPL_FILE_EXT) && !self::
                        isDir($exp . "." . DIV_DEFAULT_TPL_FILE_EXT)) {
                        ...
                        if (($_exp != '{' && $_exp != "[" && !is_numeric($_exp) && $_exp != '"' && $_exp !=
                            ...
                            substr($exp, 0 - strlen(DIV_TAG_INCLUDE_END)) == DIV_TAG_INCLUDE_END) || (substr
                            ($exp, 0, strlen(DIV_TAG_PREPROCESSED_BEGIN)) == DIV_TAG_PREPROCESSED_BEGIN &&
    ...
    final public function parseDefaults(&$items)
    {
        ...
        while (true) {
            ...
            if (!isset($arr[0]) || !isset($arr[1]))
                self::error("Was detected an invalid JSON in default values: " . substr($body, 0,
                    80) . "...", DIV_ERROR_FATAL);
            ...
            if (self::fileExists($replace) && !self::isDir($search))
                $replace = self::jsonDecode(self::getFileContents($replace), $this->getAllItems
                    ($items));
            if (self::fileExists($replace . "." . DIV_DEFAULT_DATA_FILE_EXT) && !self::
                isDir($search . "." . DIV_DEFAULT_DATA_FILE_EXT))
                $replace = self::jsonDecode(self::getFileContents($replace . "." .
                    DIV_DEFAULT_DATA_FILE_EXT), self::cop($this->__memory, $items));
            if (self::fileExists($search) && !self::isDir($search))
                $search = self::jsonDecode(self::getFileContents($search), $this->getAllItems($items));
            if (self::fileExists($search . "." . DIV_DEFAULT_DATA_FILE_EXT) && !self::isDir
                ($search . "." . DIV_DEFAULT_DATA_FILE_EXT))
                $search = self::jsonDecode(self::getFileContents($search . "." .
                    DIV_DEFAULT_DATA_FILE_EXT), $this->getAllItems($items));
    ...
    final public function scanMatch($key, $value, $engine = null, &$items = null, $ignore_logical_order = false)
    {
        ...
        if (strpos($this->__src, $key . DIV_TAG_VAR_MEMBER_DELIMITER) !== false) {
            ...
            if (is_array($vars))
                foreach ($vars as $kk => $v)
                    $this->scanMatch($key . DIV_TAG_VAR_MEMBER_DELIMITER . $kk, $v, $engine, $items,
                        $ignore_logical_order);
        }
            ...
            if ($cant_values > 0) {
                if (strpos($this->__src, $key) !== false) {
                    if (self::isNumericList($value) === true) {
                        ...
                        $this->parseMatch(DIV_TAG_AGGREGATE_FUNCTION_AVG . $sep . $key, $sum / $cant_values,
                            $engine, $ignore_logical_order);
                    }
                }

                if (strpos($this->__src, $key . DIV_TAG_AGGREGATE_FUNCTION_PROPERTY_SEPARATOR)
                    !== false) {
                    ...
                    foreach ($functions as $function) {
                        ...
                        while (true) {
                            ...
                            if ($this->searchInRanges($this->getRanges(DIV_TAG_MACRO_BEGIN,
                                DIV_TAG_MACRO_END), $range[0])) {
                                $p = $range[0] + 1;
                                continue;
                            }
        ...
        while (true) {
            ...
            if (count($ranges) > 0) {
                ...
                if (count($parts) > 0) {
                    ...
                    $newcode = DIV_TAG_STRIP_BEGIN . ' ' . DIV_TAG_TPLVAR_BEGIN . ' ' . $newvar .
                        DIV_TAG_TPLVAR_ASSIGN_OPERATOR . ' $' . $varname . ' ' . DIV_TAG_TPLVAR_END . "\n";
                    $ignore = false;
                    foreach ($parts as $part) {
                        if (trim($part) !== "") {
                            if ($part[0] == DIV_TAG_MODIFIER_TRUNCATE || $part[0] ==
                                DIV_TAG_MODIFIER_WORDWRAP || strpos($part, ",") !== false) {
                                $newcode .= DIV_TAG_TPLVAR_BEGIN . ' ' . $newvar .
                                    DIV_TAG_TPLVAR_ASSIGN_OPERATOR . ' ' . DIV_TAG_REPLACEMENT_PREFIX .
                                    DIV_TAG_MODIFIER_SIMPLE . $newvar . DIV_TAG_SUBMATCH_SEPARATOR . $part .
                                    DIV_TAG_REPLACEMENT_SUFFIX . ' ' . DIV_TAG_TPLVAR_END . "\n";
                                ...
                                $newcode .= DIV_TAG_TPLVAR_BEGIN . ' ' . $newvar .
                                    DIV_TAG_TPLVAR_ASSIGN_OPERATOR . ' ' . DIV_TAG_REPLACEMENT_PREFIX . $part . $newvar .
                                    DIV_TAG_REPLACEMENT_SUFFIX . ' ' . DIV_TAG_TPLVAR_END . "\n";
                            } else {
                                $p = $ini + 1;
                                $ignore = true;
                    ...
                    if ($ignore)
                        continue;
                    $newcode .= DIV_TAG_REPLACEMENT_PREFIX . DIV_TAG_MODIFIER_SIMPLE . $newvar .
                        DIV_TAG_REPLACEMENT_SUFFIX . ' ' . DIV_TAG_STRIP_END . "\n";
                }

                $this->__src = substr($this->__src, 0, $ini) . $newcode . substr($this->__src, $fin +
                    $l2);
                $p = $ini + 1;
            } else
                break;
    ...
    final public function parseMatches(&$items = null, $ignore_logical_order = false)
    {
        ...
        if (strpos($this->__src, DIV_TAG_LOOP_BEGIN_PREFIX) !== false) {
            ...
            while (true) {
                ...
                $ranges = $this->getBlockRanges(null, DIV_TAG_LOOP_BEGIN_PREFIX,
                    DIV_TAG_LOOP_BEGIN_SUFFIX, DIV_TAG_LOOP_END_PREFIX, DIV_TAG_LOOP_END_SUFFIX, $lastpos, null, true);
                ...
                $this->__src = substr($this->__src, 0, $ini + $lprefix + strlen($ranges[0][2]) +
                    $lsuffix) . $ukey . substr($this->__src, $ranges[0][1]);
            }
        }
    ...
    final public function parseFormulas(&$items = array())
    {
        ...
        while (true) {
            ...
            if (count($ranges) > 0) {
                ...
                if (is_null($r)) {
                    $restore_id = uniqid();
                    $this->__restore[$restore_id] = DIV_TAG_FORMULA_BEGIN . ' ' . $formula_orig .
                        DIV_TAG_FORMULA_END;
                    $this->__src = substr($this->__src, 0, $ini) . '{' . "$restore_id" . '}' .
                        substr($this->__src, $fin + $lsuffix);
                    continue;
                }

                if ($format != "" && is_numeric($r)) {
                    $this->__src = substr($this->__src, 0, $ini) . DIV_TAG_NUMBER_FORMAT_PREFIX . $random_var .
                        DIV_TAG_NUMBER_FORMAT_SEPARATOR . $format . DIV_TAG_NUMBER_FORMAT_SUFFIX .
                        substr($this->__src, $fin + $lsuffix);
                    $this->numberFormat($random_var, $r);
                } else
                    $this->__src = substr($this->__src, 0, $ini) . $r . substr($this->__src, $fin +
                        $lsuffix);
            } else
                break;
        }
    ...
    final public function parseConditions(&$items, $cleanorphan = false)
    {
        ...
        while (true) {
            $ranges = $this->getRanges(DIV_TAG_CONDITIONS_BEGIN_PREFIX,
                DIV_TAG_CONDITIONS_END, null, true, $pos);
            ...
            if (count($ranges) > 0) {
                ...
                if ($this->searchInListRanges($this->getRanges(DIV_TAG_ITERATION_BEGIN_PREFIX,
                    DIV_TAG_ITERATION_END), $ini)) {
                    $pos = $ini + 1;
                    continue;
                }
                ...
                if ($p !== false) {
                    ...
                    if ($else != false) {
                        $body_parts = array(substr($body, 0, $else), substr($body, $else + $lelse));
                    } else
                    ...
                    $this->__src = substr($this->__src, 0, $ini) . $body . substr($this->__src, $fin +
                        $lend);
                } else
                    self::error("Parse error on <b>conditions</b>: " . substr($condition, 0, 50) .
                        "...", DIV_ERROR_FATAL);
            } else
                break;
        }
    ...
    final public function parseConditional(&$items = array())
    {
        ...
        $r = array_merge($this->getBlockRanges(null,
            DIV_TAG_CONDITIONAL_TRUE_BEGIN_PREFIX, DIV_TAG_CONDITIONAL_TRUE_BEGIN_SUFFIX,
            DIV_TAG_CONDITIONAL_TRUE_END_PREFIX, DIV_TAG_CONDITIONAL_TRUE_END_SUFFIX), $this->
            getBlockRanges(null, DIV_TAG_CONDITIONAL_FALSE_BEGIN_PREFIX,
            DIV_TAG_CONDITIONAL_FALSE_BEGIN_SUFFIX, DIV_TAG_CONDITIONAL_FALSE_END_PREFIX,
            DIV_TAG_CONDITIONAL_FALSE_END_SUFFIX));
    ...
    {
        $ranges = $this->getBlockRanges($src, DIV_TAG_CONDITIONAL_FALSE_BEGIN_PREFIX,
            DIV_TAG_CONDITIONAL_FALSE_BEGIN_SUFFIX, DIV_TAG_CONDITIONAL_FALSE_END_PREFIX,
            DIV_TAG_CONDITIONAL_FALSE_END_SUFFIX);
    ...
    final public function parseDateFormat(&$items = array())
    {
        ...
        $ranges = $this->getRanges(DIV_TAG_DATE_FORMAT_PREFIX,
            DIV_TAG_DATE_FORMAT_SUFFIX);
        $vars = array();
        ...
        foreach ($ranges as $range) {
            $s = substr($this->__src, $ranges[0][0] + $lprefix, $ranges[0][1] - $ranges[0][0] -
                $lprefix);
    ...
    final public function dateFormat($key, $value)
    {
        ...
        while (true) {
            ...
            $this->__src = substr($this->__src, 0, $ini) . date($format, $value) . substr($this->
                __src, $fin + $l2);
        }
    ...
    final public function parseCapsules(&$items = array())
    {
        ...
        $pos = 0;
        while (true) {
            $ranges = $this->getBlockRanges(null, DIV_TAG_CAPSULE_BEGIN_PREFIX,
                DIV_TAG_CAPSULE_BEGIN_SUFFIX, DIV_TAG_CAPSULE_END_PREFIX,
            ...
            $this->__src = substr($this->__src, 0, $ini) . $hh . substr($this->__src, $fin +
                strlen(DIV_TAG_CAPSULE_END_PREFIX . $key . DIV_TAG_CAPSULE_END_SUFFIX));
        ...
        foreach ($objs as $obj) {
            ...
            if (array_search($method, $ms) !== false) {
                ...
                if (self::isValidMacro($params)) {
                    ...
                    self::error("Wrong params or obtrusive code in method call: $method",
                        DIV_ERROR_FATAL);
            }
        }
            ...
            if ($superkey . $key !== '')
                if (strpos($src, $superkey . $key) !== false) {
                    ...
                    if (!is_scalar($itemsx[$key]))
                        $vars = array_merge($vars, $this->getActiveVars($itemsx[$key], $superkey . $key .
                            DIV_TAG_VAR_MEMBER_DELIMITER, $src));
                }
        }
    ...
    final public function parseLocations()
    {
        ...
        foreach ($locations as $location => $posis) {
            ...
            $pos = 0;
            while (true) {
                $tag_begin = DIV_TAG_LOCATION_CONTENT_BEGIN_PREFIX . $location .
                    DIV_TAG_LOCATION_CONTENT_BEGIN_SUFFIX;
                $tag_end = DIV_TAG_LOCATION_CONTENT_END_PREFIX . $location .
                    DIV_TAG_LOCATION_CONTENT_END_SUFFIX;
                $l1 = strlen($tag_begin);
                $l2 = strlen($tag_end);
    ...
    final private function clearLocations()
    {
        $locations = $this->getLocations();
        foreach ($locations as $location => $posis) {
            $this->__src = str_replace(DIV_TAG_LOCATION_BEGIN . $location .
                DIV_TAG_LOCATION_END, '', $this->__src);
        }
    }
    ...
    final public function parseSubParsers(&$items = null, $flags = array())
    {
        ...
        foreach (self::$__sub_parsers as $parser => $function) {
            ...
            $pini = DIV_TAG_SUBPARSER_BEGIN_PREFIX . $parser .
                DIV_TAG_SUBPARSER_BEGIN_SUFFIX;
     ...
     * @return mixed
     */
    final public function checkLogicalOrder($ini = 0, $var = "", $chkloops = false,
        $chkmatchs = false, $chkformats = false, $chkdata = false)
    {
        if (self::$__log_mode)
        ...
        if ($chkloops) {
            if ($var != "")
                $prev_use = strpos($this->__src, DIV_TAG_LOOP_BEGIN_PREFIX . $var .
                    DIV_TAG_LOOP_BEGIN_SUFFIX);
            else
    ...
    final public function parseMacros(&$items = null, $ignore_previous_match = false)
    {
        ...
        while (true) {
            ...
            $this->__temp['ranges'] = $this->getRanges(DIV_TAG_MACRO_BEGIN,
                DIV_TAG_MACRO_END, null, true, $this->__temp['p']);
            ...
            if (!$ignore_previous_match) {
                $this->__temp['r'] = $this->checkLogicalOrder($this->__temp['ini'], "", true, !
                    $ignore_previous_match, true, false);
            ...
            $this->__temp['code'] = trim(substr($this->__src, $this->__temp['ini'] + $l1, $this->
                __temp['fin'] - $this->__temp['ini'] - $l1));
            $this->__temp['temp'] = uniqid();

            $this->__src = substr($this->__src, 0, $this->__temp['ini']) . '{' . $this->
                __temp['temp'] . '}' . substr($this->__src, $this->__temp['fin'] + $l2);
            ...
            if (self::isValidMacro($this->__temp['code'])) {
                ...
                $this->__temp['validmethods'] = str_replace(',', '(,' . $classname . '::', $classname .
                    '::' . $this->__temp['validmethods']) . '(';
                $this->__temp['methods'] = explode(',', str_replace($classname . '::', '', $this->
                    __temp['validmethods']));
                $this->__temp['methodsx'] = explode(',', $this->__temp['validmethods']);
                $this->__temp['code'] = str_replace($this->__temp['methods'], $this->__temp['methodsx'],
                    $this->__temp['code']);
            ...
            $this->__src = str_replace('{' . $this->__temp['temp'] . '}', ob_get_contents(),
                $this->__src);
            ...
            if ($this->__temp['invalid_macro']) {
                ...
                self::error("Invalid macro: \n\n <br/> <pre width=\"80\">" . substr($this->
                    __temp['code'], 0, 300) . '(...)</pre><br/>' . $this->__temp['details']);
            }
     ...
     * @return array
     */
    final public function saveSections($begin_prefix, $begin_suffix, $end_prefix, $end_suffix,
        $src = null)
    {
        if (is_null($src))
            $src = $this->__src;
        ...
        while (true) {
            $r = $this->getBlockRanges($src, $begin_prefix, $begin_suffix, $end_prefix, $end_suffix,
                $pos, null, true);
        ...
        return array('src' => $src, 'sections' => $saved_sections);
    }
    ...
    final private function makeItAgain($checksum, &$items)
    {
        ...
        // ... saving loops
        $r = $this->saveSections(DIV_TAG_LOOP_BEGIN_PREFIX, DIV_TAG_LOOP_BEGIN_SUFFIX,
            DIV_TAG_LOOP_END_PREFIX, DIV_TAG_LOOP_END_SUFFIX, $this->__src);
        $this->__src = $r['src'];
        $saved_sections = $r['sections'];

        // ... saving capsules
        $r = $this->saveSections(DIV_TAG_CAPSULE_BEGIN_PREFIX,
            DIV_TAG_CAPSULE_BEGIN_SUFFIX, DIV_TAG_CAPSULE_END_PREFIX,
            DIV_TAG_CAPSULE_END_SUFFIX, $this->__src);
        $this->__src = $r['src'];
        ...
        foreach (self::$__remember[$checksum] as $params) {
            ...
            switch ($params['o']) {
                case 'replace_submatch_teaser':
                    ...
                    $search = DIV_TAG_REPLACEMENT_PREFIX . $params['modifier'] . $params['key'] .
                        DIV_TAG_SUBMATCH_SEPARATOR . $params['param'] . DIV_TAG_REPLACEMENT_SUFFIX;
                ...
                case 'replace_submatch_substr':
                    ...
                    if (is_null($value))
                        continue;
                    $value = self::anyToStr($value);
                    $this->__src = str_replace($simple . $params['key'] . DIV_TAG_SUBMATCH_SEPARATOR .
                        $params['param'] . DIV_TAG_REPLACEMENT_SUFFIX, $vpx . substr($value, $params['from'],
                        $params['for']) . $vsx, $this->__src);
                    break;
                ...
                case 'replace_submatch_wordwrap':
                    ...
                    $value = self::anyToStr($value);
                    $this->__src = str_replace($simple . $params['key'] . DIV_TAG_SUBMATCH_SEPARATOR .
                        $params['param'] . DIV_TAG_REPLACEMENT_SUFFIX, $vpx . wordwrap("{$value}",
                        intval(substr($params['param'], strlen(DIV_TAG_MODIFIER_WORDWRAP))), "\n", 1) .
                        $vsx, $this->__src);
                    break;
                ...
                case 'replace_submatch_sprintf':
                    ...
                    $value = self::anyToStr($value);
                    $this->__src = str_replace($simple . $params['key'] . DIV_TAG_SUBMATCH_SEPARATOR .
                        $params['param'] . DIV_TAG_REPLACEMENT_SUFFIX, $vpx . sprintf($params['param'],
                        $value) . $vsx, $this->__src);
                    break;
                ...
                case 'json_encode':
                    ...
                    if (is_null($value))
                        continue;
                    $this->__src = str_replace(DIV_TAG_REPLACEMENT_PREFIX .
                        DIV_TAG_MODIFIER_ENCODE_JSON . $params['key'] . DIV_TAG_REPLACEMENT_SUFFIX, $vpx .
                        self::jsonEncode($value) . $vsx, $this->__src);
                    break;
                ...
                case 'simple_replacement':
                    ...
                    if ($params['before'] === false) {
                        $this->__src = str_replace(DIV_TAG_REPLACEMENT_PREFIX . $params['modifier'] . $params['key'] .
                            DIV_TAG_REPLACEMENT_SUFFIX, $vpx . $value . $vsx, $this->__src);
                    } else {
                        $substr = substr($this->__src, 0, $params['before']);
                        $substr = str_replace(DIV_TAG_REPLACEMENT_PREFIX . $params['modifier'] . $params['key'] .
                            DIV_TAG_REPLACEMENT_SUFFIX, $vpx . $value . $vsx, $substr);
     ...
     * @return string
     */
    final public function parse($from_original = true, $index_item = null, $min_level =
        1)
    {
        ...
        if (trim($this->__src) != "") {
            ...
            if (strpos($this->__src, DIV_TAG_IGNORE_BEGIN) !== false || strpos($this->__src,
                '{' . $this->__ignore_secret_tag . '}') !== false)
                $this->parseIgnore();
            ...
            $msg_infinite_cycle =
                'Too many iterations of the parser: possible infinite cycle. Review your template code.';
            ...
            do {
                ...
                do {
                    ...
                    if (self::$__log_mode === true) {
                        ...
                        if (isset($this->__src[100]))
                            $this->logger('Template [checksum=' . $checksum . ']:' . htmlentities(str_replace
                                ("\n", " ", substr($this->__src, 0, 100)) . "..." . substr($this->__src, strlen
                                ($this->__src) - 100)));
                        else
                            $this->logger('Template [checksum=' . $checksum . ']: ' . htmlentities($this->
                                __src));
                    }
                    ...
                    if (strpos($this->__src, DIV_TAG_MULTI_MODIFIERS_PREFIX) !== false && strpos($this->
                        __src, DIV_TAG_MULTI_MODIFIERS_SUFFIX) !== false)
                    ...
                    // Matches
                    if (self::atLeastOneString($this->__src, self::$__modifiers) || (strpos($this->
                        __src, DIV_TAG_NUMBER_FORMAT_PREFIX) !== false && strpos($this->__src,
                        DIV_TAG_NUMBER_FORMAT_SUFFIX) !== false))
            ...
            if (strpos($this->__src, DIV_TAG_LOCATION_BEGIN) !== false) {
                $allitems = $this->getAllItems($items);
                $clear = self::getVarValue('div' . DIV_TAG_VAR_MEMBER_DELIMITER .
                    'clear_locations', $allitems);
            ...
            if (self::$__parse_level <= 1) {
                ...
                foreach (self::$__ignored_parts as $id => $ignore) {
                    ...
                    foreach (self::$__sub_parsers as $subparser => $function) {
                        ...
                        $tagsearch = DIV_TAG_SUBPARSER_BEGIN_PREFIX . $subparser .
                            DIV_TAG_SUBPARSER_BEGIN_SUFFIX;
                        $tagreplace = DIV_TAG_SUBPARSER_BEGIN_PREFIX . $tempunique .
                            DIV_TAG_SUBPARSER_BEGIN_SUFFIX;
        ...
        if (strpos($this->__src, DIV_TAG_SUBPARSER_BEGIN_PREFIX) !== false) {
            $items = array_merge($this->__memory, $items);
            $this->parseSubParsers($items, array('moment' => DIV_MOMENT_AFTER_PARSE, 'level' =>
                    self::$__parse_level));
            $this->memory($items);
        }
        ...
        if (self::$__log_mode)
            $this->logger("Parser duration: " . number_format($time_end - $time_start, 5) .
                " secs");
    ...
    final public function parseSpecialChars()
    {
        $this->__src = str_replace(DIV_TAG_SPECIAL_REPLACE_NEW_LINE . "\n\n",
            DIV_TAG_SPECIAL_REPLACE_NEW_LINE . "\n", $this->__src);
        $this->__src = str_replace(DIV_TAG_SPECIAL_REPLACE_NEW_LINE, "\n", $this->__src);
        $this->__src = str_replace(DIV_TAG_SPECIAL_REPLACE_HORIZONTAL_TAB, "\t", $this->
            __src);
        $this->__src = str_replace(DIV_TAG_SPECIAL_REPLACE_CAR_RETURN, "\r", $this->
            __src);
        $this->__src = str_replace(DIV_TAG_SPECIAL_REPLACE_VERTICAL_TAB, "\v", $this->
            __src);
        $this->__src = str_replace(DIV_TAG_SPECIAL_REPLACE_NEXT_PAGE, "\f", $this->
            __src);
        $this->__src = str_replace(DIV_TAG_SPECIAL_REPLACE_DOLLAR_SYMBOL, "\$", $this->
            __src);
    }
            ...
            foreach ($items as $key => $value) {
                if (self::isArrayOfArray($value)) {
                    ...
                    while (true) {
                        ...
                        $this->__src = substr($this->__src, 0, $ini) . $subsrc . substr($this->__src, $fin +
                            $l);
                    }
                }
            }
     ...
     * @param string $end
     */
    final private function translateSimpleBlocks($src, $begin, $end, $tbegin, $tend,
        $separator = "", $tseparator = "", $first = true)
    {
        $lbegin = strlen($begin);
        $lend = strlen($end);
     ...
     */
    final private function translateKeyBlocks($src, $begin_prefix, $begin_suffix, $end_prefix,
        $end_suffix, $tbegin_prefix, $tbegin_suffix, $tend_prefix, $tend_suffix, $var_member_delimiter,
        $tvar_member_delimiter)
    {
        $p = 0;
        while (true) {

            $r = $this->getBlockRanges($src, $begin_prefix, $begin_suffix, $end_prefix, $end_suffix,
                $p, null, true, $var_member_delimiter);
            ...
            $src = substr($src, 0, $ini) . $tbegin_prefix . $key . $tbegin_suffix . $subsrc .
                $tend_prefix . $key . $tend_suffix . substr($src, $end + $lsuffix);
    ...
    final public function translateFrom($dialectFrom, $src = null, $items = null)
    {
        ...
        $agfuncs = array(
            DIV_TAG_AGGREGATE_FUNCTION_COUNT . $temp => $DIV_TAG_AGGREGATE_FUNCTION_COUNT .
                $temp,
        ...
        foreach ($order as $o => $priority) {
            switch ($o) {
                case 'replacement':
                    foreach ($xmod as $modifier => $values) {
                        ...
                        while (true) {
                            ...
                            $r = $this->getRanges($DIV_TAG_REPLACEMENT_PREFIX . $values[0], $DIV_TAG_REPLACEMENT_SUFFIX,
                                $src, true, $p);
                            if (count($r) < 1)
                                break;
                            ...
                            // Aggregate functions
                            $subsrc = str_replace($DIV_TAG_AGGREGATE_FUNCTION_PROPERTY_SEPARATOR,
                                DIV_TAG_AGGREGATE_FUNCTION_PROPERTY_SEPARATOR, $subsrc);
                            $subsrc = str_replace($agfuncs, $agfuncs_keys, $subsrc);

                            // Teaser or truncate
                            $subsrc = str_replace($DIV_TAG_SUBMATCH_SEPARATOR . $DIV_TAG_MODIFIER_TRUNCATE,
                                DIV_TAG_SUBMATCH_SEPARATOR . DIV_TAG_MODIFIER_TRUNCATE, $subsrc);

                            // Word wrap
                            $subsrc = str_replace($DIV_TAG_SUBMATCH_SEPARATOR . $DIV_TAG_MODIFIER_WORDWRAP,
                                DIV_TAG_SUBMATCH_SEPARATOR . DIV_TAG_MODIFIER_WORDWRAP, $subsrc);

                            // Format (sprintf)
                            $subsrc = str_replace($DIV_TAG_SUBMATCH_SEPARATOR . $DIV_TAG_MODIFIER_FORMAT,
                            ...
                            // Member delimiters
                            $subsrc = str_replace($DIV_TAG_VAR_MEMBER_DELIMITER,
                                DIV_TAG_VAR_MEMBER_DELIMITER, $subsrc);

                            $src = substr($src, 0, $ini) . DIV_TAG_REPLACEMENT_PREFIX . $values[1] . $subsrc .
                                DIV_TAG_REPLACEMENT_SUFFIX . substr($src, $end + $lsuffix);
                ...
                case 'multimodifiers':
                    ...
                    $p = 0;
                    while (true) {
                        $r = $this->getRanges($DIV_TAG_MULTI_MODIFIERS_PREFIX, $DIV_TAG_MULTI_MODIFIERS_SUFFIX,
                            $src, true, $p);
                        if (count($r) < 1)
                            break;
                        ...
                        $subsrc = str_replace($DIV_TAG_VAR_MEMBER_DELIMITER,
                            DIV_TAG_VAR_MEMBER_DELIMITER, substr($subsrc, 0, $po)) .
                            DIV_TAG_MULTI_MODIFIERS_OPERATOR . $temp;

                        // Member delimiters
                        $subsrc = str_replace($DIV_TAG_VAR_MEMBER_DELIMITER,
                            DIV_TAG_VAR_MEMBER_DELIMITER, $subsrc);

                        $src = substr($src, 0, $ini) . DIV_TAG_MULTI_MODIFIERS_PREFIX . $subsrc .
                            DIV_TAG_MULTI_MODIFIERS_SUFFIX . substr($src, $end + $lsuffix);
                        $p = $ini + 1; // IMPORTANT!
                    }
                    break;
                case 'dateformat':
                    ...
                    $p = 0;
                    while (true) {
                        $r = $this->getRanges($DIV_TAG_DATE_FORMAT_PREFIX, $DIV_TAG_DATE_FORMAT_SUFFIX,
                            $src, true, $p);
                        if (count($r) < 1)
                            break;
                        ...
                        if ($po !== false)
                            $subsrc = str_replace($DIV_TAG_VAR_MEMBER_DELIMITER,
                                DIV_TAG_VAR_MEMBER_DELIMITER, substr($subsrc, 0, $po)) .
                                DIV_TAG_DATE_FORMAT_SEPARATOR . substr($subsrc, $po + $lsep);

                        $src = substr($src, 0, $ini) . DIV_TAG_DATE_FORMAT_PREFIX . $subsrc .
                            DIV_TAG_DATE_FORMAT_SUFFIX . substr($src, $end + $lsuffix);
                        $p = $ini + 1; // IMPORTANT!
                    }
                    break;
                case 'numberformat':
                    ...
                    $p = 0;
                    while (true) {
                        $r = $this->getRanges($DIV_TAG_NUMBER_FORMAT_PREFIX, $DIV_TAG_NUMBER_FORMAT_SUFFIX,
                            $src, true, $p);
                        if (count($r) < 1)
                            break;
                        ...
                        if ($po !== false)
                            $subsrc = str_replace($DIV_TAG_VAR_MEMBER_DELIMITER,
                                DIV_TAG_VAR_MEMBER_DELIMITER, substr($subsrc, 0, $po)) .
                                DIV_TAG_NUMBER_FORMAT_SEPARATOR . substr($subsrc, $po + $lsep);

                        $src = substr($src, 0, $ini) . DIV_TAG_NUMBER_FORMAT_PREFIX . $subsrc .
                            DIV_TAG_NUMBER_FORMAT_SUFFIX . substr($src, $end + $lsuffix);
                        $p = $ini + 1; // IMPORTANT!
                    }
                ...
                case 'formulas':
                    $src = $this->translateSimpleBlocks($src, $DIV_TAG_FORMULA_BEGIN, $DIV_TAG_FORMULA_END,
                        ...
                        DIV_TAG_FORMULA_FORMAT_SEPARATOR, false);
                    break;
                case 'subparsers':

                    foreach (self::$__sub_parsers as $subparser => $function) {
                        $src = str_replace($DIV_TAG_SUBPARSER_BEGIN_PREFIX . $subparser . $DIV_TAG_SUBPARSER_BEGIN_SUFFIX,
                            DIV_TAG_SUBPARSER_BEGIN_PREFIX . $subparser . DIV_TAG_SUBPARSER_BEGIN_SUFFIX, $src);
                        $src = str_replace($DIV_TAG_SUBPARSER_END_PREFIX . $subparser . $DIV_TAG_SUBPARSER_END_SUFFIX,
                            DIV_TAG_SUBPARSER_END_PREFIX . $subparser . DIV_TAG_SUBPARSER_END_SUFFIX, $src);
                    }

                    break;
                case 'ignore':
                    $src = $this->translateSimpleBlocks($src, $DIV_TAG_IGNORE_BEGIN, $DIV_TAG_IGNORE_END,
                        DIV_TAG_IGNORE_BEGIN, DIV_TAG_IGNORE_END);
                    break;

                case 'comment':
                    $src = $this->translateSimpleBlocks($src, $DIV_TAG_COMMENT_BEGIN, $DIV_TAG_COMMENT_END,
                        DIV_TAG_COMMENT_BEGIN, DIV_TAG_COMMENT_END);
                    break;
                ...
                case 'html2txt':
                    ...
                    while (true) {
                        ...
                        if ($po !== false)
                            $subsrc = substr($subsrc, 0, $po) . DIV_TAG_TXT_WIDTH_SEPARATOR . substr($subsrc,
                                $po + $lsep);

                        $src = substr($src, 0, $ini) . DIV_TAG_TXT_BEGIN . $subsrc . DIV_TAG_TXT_END .
                    ...
                    break;
                case 'strip':
                    $src = $this->translateSimpleBlocks($src, $DIV_TAG_STRIP_BEGIN, $DIV_TAG_STRIP_END,
                ...
                case 'loops':
                    ...
                    while (true) {
                        ...
                        $r = $this->getBlockRanges($src, $DIV_TAG_LOOP_BEGIN_PREFIX, $DIV_TAG_LOOP_BEGIN_SUFFIX,
                            $DIV_TAG_LOOP_END_PREFIX, $DIV_TAG_LOOP_END_SUFFIX, $p, null, true, $DIV_TAG_VAR_MEMBER_DELIMITER);
                        ...
                        if ($po !== false)
                            $subsrc = substr($subsrc, 0, $po) . DIV_TAG_LOOP_VAR_SEPARATOR . substr($subsrc,
                                $po + $lsep);

                        $subsrc = str_replace(array($DIV_TAG_EMPTY, $DIV_TAG_BREAK), array(DIV_TAG_EMPTY,
                                DIV_TAG_BREAK), $subsrc);

                        $key = str_replace($DIV_TAG_VAR_MEMBER_DELIMITER, DIV_TAG_VAR_MEMBER_DELIMITER,
                            $key);
                        $src = substr($src, 0, $ini) . DIV_TAG_LOOP_BEGIN_PREFIX . $key .
                            DIV_TAG_LOOP_BEGIN_SUFFIX . $subsrc . DIV_TAG_LOOP_END_PREFIX . $key .
                            DIV_TAG_LOOP_END_SUFFIX . substr($src, $end + $lsuffix);
                ...
                case 'iterations':
                    ...
                    while (true) {
                        ...
                        $src = substr($src, 0, $ini) . DIV_TAG_ITERATION_BEGIN_PREFIX . $s .
                            DIV_TAG_ITERATION_BEGIN_SUFFIX . $subsrc . DIV_TAG_ITERATION_END . substr($src,
                            $end + $lend);
                ...
                case 'conditionals':
                    ...
                    while (true) {
                        ...
                        $r = $this->getBlockRanges($src, $DIV_TAG_CONDITIONAL_TRUE_BEGIN_PREFIX, $DIV_TAG_CONDITIONAL_TRUE_BEGIN_SUFFIX,
                            $DIV_TAG_CONDITIONAL_TRUE_END_PREFIX, $DIV_TAG_CONDITIONAL_TRUE_END_SUFFIX, $p, null, true,
                            $DIV_TAG_VAR_MEMBER_DELIMITER);
                        ...
                        $key = str_replace($DIV_TAG_VAR_MEMBER_DELIMITER, DIV_TAG_VAR_MEMBER_DELIMITER,
                            $key);
                        $src = substr($src, 0, $ini) . DIV_TAG_CONDITIONAL_TRUE_BEGIN_PREFIX .
                            str_replace($DIV_TAG_VAR_MEMBER_DELIMITER, DIV_TAG_VAR_MEMBER_DELIMITER, $key) .
                            DIV_TAG_CONDITIONAL_TRUE_BEGIN_SUFFIX . $subsrc .
                    ...
                    while (true) {
                        ...
                        $r = $this->getBlockRanges($src, $DIV_TAG_CONDITIONAL_FALSE_BEGIN_PREFIX, $DIV_TAG_CONDITIONAL_FALSE_BEGIN_SUFFIX,
                            $DIV_TAG_CONDITIONAL_FALSE_END_PREFIX, $DIV_TAG_CONDITIONAL_FALSE_END_SUFFIX, $p, null, true,
                            $DIV_TAG_VAR_MEMBER_DELIMITER);
                        ...
                        $key = str_replace($DIV_TAG_VAR_MEMBER_DELIMITER, DIV_TAG_VAR_MEMBER_DELIMITER,
                            $key);
                        $src = substr($src, 0, $ini) . DIV_TAG_CONDITIONAL_FALSE_BEGIN_PREFIX . $key .
                            DIV_TAG_CONDITIONAL_FALSE_BEGIN_SUFFIX . $subsrc .
                            DIV_TAG_CONDITIONAL_FALSE_END_PREFIX . $key .
                            DIV_TAG_CONDITIONAL_FALSE_END_SUFFIX . substr($src, $end + $lsuffix);
                ...
                case 'conditions':
                    ...
                    $p = 0;
                    while (true) {
                        $r = $this->getRanges($DIV_TAG_CONDITIONS_BEGIN_PREFIX, $DIV_TAG_CONDITIONS_END,
                            $src, true, $p);
                        ...
                        $src = substr($src, 0, $ini) . DIV_TAG_CONDITIONS_BEGIN_PREFIX . $s .
                            DIV_TAG_CONDITIONS_BEGIN_SUFFIX . $subsrc . DIV_TAG_CONDITIONS_END . substr($src,
                            $end + $lend);
                ...
                case 'tplvars':
                    ...
                    while (true) {
                        ...
                        foreach ($missing_vars as $missing)
                            $tplvarvalue = str_replace('$' . $missing, '$' . str_replace($DIV_TAG_VAR_MEMBER_DELIMITER,
                                DIV_TAG_VAR_MEMBER_DELIMITER, $missing), $tplvarvalue);

                        $src = substr($src, 0, $ini) . DIV_TAG_TPLVAR_BEGIN . str_replace($DIV_TAG_TPLVAR_PROTECTOR,
                            DIV_TAG_TPLVAR_PROTECTOR, str_replace($DIV_TAG_VAR_MEMBER_DELIMITER,
                            DIV_TAG_VAR_MEMBER_DELIMITER, substr($src, $ini + $lbegin, $p1 - ($ini + $lbegin)))) .
                            DIV_TAG_TPLVAR_ASSIGN_OPERATOR . $tplvarvalue . DIV_TAG_TPLVAR_END . substr($src,
                            $end + $lend);
                ...
                case 'defaultreplace':
                    $src = $this->translateSimpleBlocks($src, $DIV_TAG_DEFAULT_REPLACEMENT_BEGIN, $DIV_TAG_DEFAULT_REPLACEMENT_END,
                        DIV_TAG_DEFAULT_REPLACEMENT_BEGIN, DIV_TAG_DEFAULT_REPLACEMENT_END);
                    break;

                case 'include':
                    $src = $this->translateSimpleBlocks($src, $DIV_TAG_INCLUDE_BEGIN, $DIV_TAG_INCLUDE_END,
                        DIV_TAG_INCLUDE_BEGIN, DIV_TAG_INCLUDE_END);
                    break;

                case 'preprocessed':
                    $src = $this->translateSimpleBlocks($src, $DIV_TAG_PREPROCESSED_BEGIN, $DIV_TAG_PREPROCESSED_END,
                        DIV_TAG_PREPROCESSED_BEGIN, DIV_TAG_PREPROCESSED_END,
                        DIV_TAG_PREPROCESSED_SEPARATOR);
                    break;

                case 'capsules':
                    $src = $this->translateKeyBlocks($src, $DIV_TAG_CAPSULE_BEGIN_PREFIX, $DIV_TAG_CAPSULE_BEGIN_SUFFIX,
                        $DIV_TAG_CAPSULE_END_PREFIX, $DIV_TAG_CAPSULE_END_SUFFIX,
                        DIV_TAG_CAPSULE_BEGIN_PREFIX, DIV_TAG_CAPSULE_BEGIN_SUFFIX,
                        DIV_TAG_CAPSULE_END_PREFIX, DIV_TAG_CAPSULE_END_SUFFIX, $DIV_TAG_VAR_MEMBER_DELIMITER,
                        DIV_TAG_VAR_MEMBER_DELIMITER);
                    break;

                case 'multireplace':
                    $src = $this->translateKeyBlocks($src, $DIV_TAG_MULTI_REPLACEMENT_BEGIN_PREFIX,
                        $DIV_TAG_MULTI_REPLACEMENT_BEGIN_SUFFIX, $DIV_TAG_MULTI_REPLACEMENT_END_PREFIX,
                        $DIV_TAG_MULTI_REPLACEMENT_END_SUFFIX, DIV_TAG_MULTI_REPLACEMENT_BEGIN_PREFIX,
                        DIV_TAG_MULTI_REPLACEMENT_BEGIN_SUFFIX, DIV_TAG_MULTI_REPLACEMENT_END_PREFIX,
                        DIV_TAG_MULTI_REPLACEMENT_END_SUFFIX, $DIV_TAG_VAR_MEMBER_DELIMITER,
                        DIV_TAG_VAR_MEMBER_DELIMITER);
                    break;

                case 'friendlytags':
                    $src = $this->translateSimpleBlocks($src, $DIV_TAG_FRIENDLY_BEGIN, $DIV_TAG_FRIENDLY_END,
                        DIV_TAG_FRIENDLY_BEGIN, DIV_TAG_FRIENDLY_END);
                    break;

                case 'macros':
                    $src = $this->translateSimpleBlocks($src, $DIV_TAG_MACRO_BEGIN, $DIV_TAG_MACRO_END,
                        DIV_TAG_MACRO_BEGIN, DIV_TAG_MACRO_END);
                    break;

                case 'location':
                    $src = $this->translateSimpleBlocks($src, $DIV_TAG_LOCATION_BEGIN, $DIV_TAG_LOCATION_END,
                        DIV_TAG_LOCATION_BEGIN, DIV_TAG_LOCATION_END);
                    break;

                case 'locontent':
                    $src = $this->translateKeyBlocks($src, $DIV_TAG_LOCATION_CONTENT_BEGIN_PREFIX, $DIV_TAG_LOCATION_CONTENT_BEGIN_SUFFIX,
                        ...
                        DIV_TAG_LOCATION_CONTENT_END_PREFIX, DIV_TAG_LOCATION_CONTENT_END_SUFFIX, $DIV_TAG_VAR_MEMBER_DELIMITER,
                        DIV_TAG_VAR_MEMBER_DELIMITER);
                    break;
            }
        }
     ...
     * @return string
     */
    final static function div($src = null, $items = null, $ignore = array(), $min_level =
        1)
    {
        $class = get_class();
    ...
    final static function getDocsReadable($tpl = null, $items = null)
    {
        ...
        if (is_null($items))
            $items = array('title' => "Templates's documentation");
    ...
    final static function utf162utf8($utf16)
    {
        ...
        if ((0xFFFF & $bytes) == $bytes)
            return chr(0xE0 | (($bytes >> 12) & 0x0F)) . chr(0x80 | (($bytes >> 6) & 0x3F)) .
                chr(0x80 | ($bytes & 0x3F));
                    ...
                    for ($c = 0; $c < $strlen_chrs; ++$c) {
                        ...
                        switch (true) {
                            ...
                            case preg_match('/\\\u[0-9A-F]{4}/i', substr($chrs, $c, 6)):
                                $utf16 = chr(hexdec(substr($chrs, ($c + 2), 2))) . chr(hexdec(substr($chrs, ($c +
                                    4), 2)));
                    ...
                    for ($c = 0; $c <= $strlen_chrs; ++$c) {
                        ...
                        if (($c == $strlen_chrs) || (($chrs{$c} == ',') && ($top['what'] == 1))) {
                            ...
                            array_push($stk, array(
                                ...
                                'where' => $c,
                                'delim' => $chrs{$c}));
                        } elseif (($chrs{$c} == $top['delim']) && ($top['what'] == 2) && ((strlen(substr
    ...
    final static function jsonEncode($data)
    {
        if (is_array($data) || is_object($data)) {
            $islist = is_array($data) && (empty($data) || array_keys($data) === range(0,
                count($data) - 1));
            ...
            for ($i = 0; $i < $len; $i++) {
                ...
                $c3 = ord($string[++$i]);
                if (($c1 & 16) === 0) {
                    $json .= sprintf("\\u%04x", (($c1 - 224) << 12) + (($c2 - 128) << 6) + ($c3 -
                        128));
                    continue;
                }
                $c4 = ord($string[++$i]);
    ...
    {
        if (substr(strtolower($filename), 0, 7) == 'http://' || substr(strtolower($filename),
            0, 8) == 'https://' || substr(strtolower($filename), 0, 6) == 'ftp://')
            return false;
    ...
    {
        if (substr(strtolower($dirname), 0, 7) == 'http://' || substr(strtolower($dirname),
            0, 8) == 'https://' || substr(strtolower($dirname), 0, 6) == 'ftp://')
            return false;
        ...
        '$_REQUEST'), $allow_classes = false, $allow_functions = false)
    {
        ...
        if (is_null($valid_tokens))
            $valid_tokens = DIV_PHP_VALID_TOKENS_FOR_EXPRESSIONS . ',' .
                DIV_PHP_VALID_TOKENS_FOR_MACROS;
    ...
    {
        return self::isValidPHPCode($code, true, DIV_PHP_VALID_TOKENS_FOR_EXPRESSIONS .
            ',' . DIV_PHP_VALID_TOKENS_FOR_MACROS);
    }
    ...
    final static function isCli()
    {
        if (is_null(self::$__is_cli)) {
            self::$__is_cli = (!isset($_SERVER['SERVER_SOFTWARE']) && (php_sapi_name() ==
    ...
    static function error($errmsg, $level = DIV_ERROR_WARNING)
    {
        ...
        if ($iscli === false)
            echo "<div style = \"z-index:9999; position: absolute; top: " . ((count(self::$__errors) -
                1) * 50 + 10) . "px; right: 20px; width: 600px;max-height: 600px; overflow:auto;; font-family: courier; padding: 10px;";
