...
if( ! defined('DIV_TAG_SPECIAL_REPLACE_SPACE')) define('DIV_TAG_SPECIAL_REPLACE_SPACE', '{\s}');
...
class div{
	...
	public function __construct($src = null, $items = null, $ignore = []){
		...
		if(self::$__super_class === null) self::$__super_class = $this->getSuperParent();

		if(self::$__parent_method_names === null) self::$__parent_method_names = get_class_methods(self::$__super_class);
		...
		if($items === null && $this->__items !== null) $items = $this->__items;
		...
		if($src === null){
			if($class_name != self::$__super_class && $this->__src === null) $src = $class_name;
			if($this->__src !== null) $src = $this->__src;
		}
	...
	final public function getSuperParent($class_name = null){
		if($class_name === null) $class_name = get_class($this);
	...
	final static function createAuxiliaryEngine(&$from = null){
		...
		if(self::$__engine !== null) if(get_class(self::$__engine) != $class_name) self::$__engine = null;
		...
		if(self::$__engine === null){
			...
			if($from !== null) // auxiliary engine have optional items
			{
	...
	final static function getAuxiliaryEngineClone(&$items = null, &$items_orig = null, $from = null, $src = null, $src_orig = null){
		if($from !== null){
			if(self::$__engine !== null && get_class($from) == get_class(self::$__engine)){
				$obj = clone self::$__engine;
			}
			else{
				$temp_engine = null;
				if(self::$__engine !== null) $temp_engine = clone self::$__engine;
			...
			if(self::$__engine === null) self::createAuxiliaryEngine($from);
			$obj = clone self::$__engine;
		}

		if(self::$__log_mode) self::log("getAuxiliaryEngineClone: New auxiliary #" . $obj->getId());

		if($items !== null) $obj->__items = $items;
		if($items_orig !== null) $obj->__items_orig = $items_orig;
		if($src !== null) $obj->__src = $items;
		if($src_orig !== null) $obj->__src_orig = $items_orig;
	...
	final public function getRanges($tag_ini, $tag_end, $src = null, $only_first = false, $pos = 0){
		$ranges = [];
		if($src === null) if(isset ($this)) $src = $this->__src;
		if($src !== null){
	...
	final public function getBlockRanges($src = null, $begin_prefix = '{', $begin_suffix = '}', $end_prefix = '{/', $end_suffix = '}', $after = 0, $before = null, $only_first = false, $var_member_delimiter = DIV_TAG_VAR_MEMBER_DELIMITER){
		if($src === null) $src = $this->__src;
		if($before !== null) $src = substr($src, 0, $before);
	...
	final public function searchInListRanges($pos = 0){
		...
		foreach($ranges as $rang){
			if($pos > $rang [0] && $pos < $rang [1]) return true;
		}
	...
	final public function parseSubmatches(&$items = null){
		...
		foreach(self::$__modifiers as $m){
			if(strpos($this->__src, DIV_TAG_REPLACEMENT_PREFIX . $m) !== false) $modifiers [] = $m;
		}
	...
	final public function addLiteral($var){
		...
		if($literals === null || $literals === false) $literals = [];
	...
	final public function isBlockInsideBlock($tag_ini, $tag_end, $pos1, $pos2, $ranges = null){
		if($ranges === null) $ranges = $this->getRanges($tag_ini, $tag_end);

		foreach($ranges as $rang){
			if($rang [0] < $pos1 && $rang [1] > $pos2) return true;
		}
				...
				if( ! empty($value)){
					...
					foreach($xitems as $xkey => $item){
						...
						foreach($item as $kkk => $vvv){
							if(isset (self::$__globals_design [$kkk])) unset (self::$__globals_design [$kkk]);
						}
						...
						foreach($item as $kkk => $vvv){
							if(isset ($engine->__memory [$kkk])) unset ($engine->__memory [$kkk]);
						}
						...
						foreach($item as $kkk => $vvv){
							if(isset ($this->__memory [$kkk])) unset ($this->__memory [$kkk]);
						}
	...
	final public function parseIgnore(){
		...
		// Generate internal and random ignore tag (security reasons)
		if($this->__ignore_secret_tag === null) $this->__ignore_secret_tag = uniqid();
	...
	final public function parseComments($section = null){
		if($section === null) $section = trim($this->__path);
	...
	final public function getVars($items = null, $super_key = ''){
		if($items === null) $items = $this->__memory;
		...
		foreach($items_x as $key => $value){
			...
			if($x_key !== ''){
				...
				if( ! is_scalar($value) && $value !== null) $vars = array_merge($vars, $this->getVars($items_x [$key], $x_key . DIV_TAG_VAR_MEMBER_DELIMITER));
			}
		}
	...
	final public function scanMatch($key, $value, $engine = null, &$items = null, $ignore_logical_order = false){
		if($items === null) $items = &$this->__items;
		if($engine === null) $engine = self::getAuxiliaryEngineClone($items, $items, $this);
									...
									if(isset ($v [$var])){
										...
										switch($function){
											case DIV_TAG_AGGREGATE_FUNCTION_MIN :
												if($min * 1 > $v [$var] * 1 || $min === null) $min = $v [$var] * 1;
												break;
											case DIV_TAG_AGGREGATE_FUNCTION_MAX :
												if($max * 1 < $v [$var] * 1 || $max === null) $max = $v [$var] * 1;
												break;
	...
	final public function getActiveVars($items, $super_key = '', $src = null){
		...
		foreach($items_x as $key => $value){
			if($super_key . $key !== ''){
				if(strpos($src, $super_key . $key) !== false){
					$vars [] = $super_key . $key;
		...
		foreach($locations as $location => $positions){
			...
			while(true){
				...
				foreach($locations as $k => $v){
					foreach($v as $kk => $p){
						if($p > $end) $locations [$k] [$kk] -= $l1 + $l2 + ($end - $ini - $l1);
					}
				}
			}
	...
	final public function prepareDialect($src = null, $properties = null, $update = true){
		...
		if($properties === null) $properties = $this->__properties;
	...
	final public function translate($dialectFrom = [], $dialectTo = [], $src = null, $items = null){
		...
		foreach($constants as $c => $v){
			if(substr($c, 0, 8) == 'DIV_TAG_') $new_constants [$c] = $v;
		}
	...
	final static function getDocsReadable($tpl = null, $items = null){
		...
		if($tpl === null) $tpl = DIV_TEMPLATE_FOR_DOCS;
	...
	final static function cop(&$source, $complement, $level = 0){
		...
		if($source === null) return $complement;
		if($complement === null) return $source;
	...
	static function htmlToText($html, $width = 50){
		...
		$html = trim($html);
		if($width !== null && $width !== 0) $html = wordwrap($html, $width, "\n");
	 ...
	 * @return bool
	 */
	final static function haveVarsThisCode($code, $ignore = []){
	...
	final static function isValidPHPCode(
		...
		if($valid_tokens === null) $valid_tokens = DIV_PHP_VALID_TOKENS_FOR_EXPRESSIONS . ',' . DIV_PHP_VALID_TOKENS_FOR_MACROS;
	...
	static function getErrorReporting(){
		if(self::$__error_reporting === null) self::$__error_reporting = E_ALL;
