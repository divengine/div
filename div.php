...
class div
{
        ...
        if ($is_string || is_numeric($value)) {
            ...
            if ($px !== false) {
                ...
                foreach (self::$__custom_modifiers as $modifier) {
                    ...
                    // new from 5.0: closure functions as custom modifiers
                    if (is_object($modifier[1])) {
                        if (get_class($modifier[1]) == "Closure")
                            if (strpos($substr, $modifier [0]) !== false) {
                                $vvalue = $modifier[1]($value);
        ...
        while (true) {
            ...
            if (!self::fileExists($path)) {
                ...
                if ($sep !== false) {
                    ...
                    if (self::varExists($pdata, $allitems)) {
                    ...
                    elseif (self::fileExists($pdata . "." . DIV_DEFAULT_DATA_FILE_EXT)) {
        ...
        while (true) {
            ...
            if ( ! self::fileExists($path)) {
                ...
                if ($sep !== false) {
                    ...
                    $standalone = self::getVarValue("div.standalone", $pdata);
    ...
    final public function parseConditional(&$items = array())
    {
        ...
        foreach ($r as $tag) {
            ...
            $otag = $tag;
            $tag [2] = self::div($tag [2], $ii, array(), 1, true);
     ...
     * @return string
     */
    final static function div($src = null, $items = null, $ignore = array(), $min_level = 1, $discard_file_system = false)
    {
        $class = get_class();

        // some time $src is not a template, then discard file system can avoid infinite loops

        $discardfs = self::$__discard_file_system; // save current value
        self::$__discard_file_system = $discard_file_system;

        $engine = new $class ($src, $items, $ignore);
        $engine->parse(false, null, $min_level);

        self::$__discard_file_system = $discardfs; // restore original value
        return $engine->__src;
    }
